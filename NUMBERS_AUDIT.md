...
**Scope note (current cumulative contract).** The rules and per-source rows in this section describe the historical 15-to-25 May reporting series, when suspected was still carried as a published cumulative tier. As of the 2026-08-03 SitRep #081 promotion, the only cumulative epidemiological metrics are laboratory-confirmed cases (3894: 3874 DRC + 20 Uganda) and confirmed deaths (1753: 1751 DRC + 2 Uganda). All 51 printed DRC rows close exactly to 3874 confirmed cases but to 1749 deaths, two below the official 1751 DRC death headline because North Kivu's narrative and zone rows report 288 deaths while Table 1 reports 290. The two-death residual is retained rather than assigned to a zone without evidence. The official corridor source-load is 3874 and the 20-case attribution gap is entirely the Uganda anchor. The notified 24-hour movement is +72 confirmed and +44 deaths, with no new zone. The cumulative suspected tier and confirmed-and-suspected composite remain archived from the headline. Suspected cases appear only as point-in-time operational caseloads on a separate axis. SitRep #081 reports 717 patients in isolation, classifying 330 as confirmed and 356 as suspected across four provinces; Haut-Uele's remaining 31 patients are not split by status and remain unclassified. The under-investigation stock and missing status assignments are not fabricated.
...
The 457-corridor watchlist is emitted from the INSP per-health-zone surface: 51 official source zones crossed with nine candidate targets, minus the goma-cod and beni-cod self-edges. For the 2026-08-03 snapshot, adjusted 50% ranges span 0.5-99.8% lower and 1.5-100.0% upper; bunia -> goma-cod ranks first at 99.8-100.0%. The country headline is 3894 confirmed cases (3874 DRC + 20 Uganda); 3874 confirmed cases are officially zone-attributed, while 20 confirmed cases remain an explicit source-attribution lag comprising the Uganda anchor alone. No zone enters or leaves this cycle. Audit refs: `ec:lovs:method:bdbv-zone-attributed-corridors:2026-05-22`; `ec:lovs:data:inrb-sitrep-081-visual-promotion:2026-08-03`. Numbers below remain the pre-committed calibration corridors and are not re-derived from future rankings.
