# Numbers audit, snapshot series through 13 June 2026
...
This file is the **single ground-truth registry** for the dated snapshot series through 13 June 2026. If a downstream number does not appear here, or appears with a different attribution, that is a bug. Update this file in the same commit that updates the number.
...
**Scope note (current cumulative contract).** The rules and per-source rows in this section describe the historical 15-to-25 May reporting series, when suspected was still carried as a published cumulative tier. As of the 2026-06-13 SitRep #030 promotion, the only cumulative epidemiological metrics are laboratory-confirmed cases (801 confirmed cases: 782 DRC + 19 Uganda) and confirmed deaths (183: 181 DRC + 2 Uganda). The cumulative suspected tier and the confirmed-and-suspected composite are paused and archived from the headline surface (retained as dated provenance, and reactivatable in a future snapshot). Suspected cases now appear only as point-in-time operational caseloads such as suspected cases in isolation on a separate non-cumulative axis, and the per-source suspected rows below are retained as archival provenance of what each publisher reported on its own date, never summed into confirmed. SitRep #030 does not republish the under-investigation stock or total active suspected queue, so those values are not fabricated. The present-tense "current LOVS contract" phrasing in the rules below is preserved as the historical record for that series.
...
|---|---|---|---|---|
| Reporting completeness 50% lower | 0.495 (49.5%) | lovs/lovs_visibility.py nowcast() | reported_counts (above), affected_zones, snapshot.as_of | Bottom-line aside; AtAGlance visibility row; InferredTrajectory band; brief Ascertainment section "49 to 57 percent"; README headline finding; Audit ref: `ec:lovs:grepi:reporting-delay-update:2026-05-23` |
| Reporting completeness 50% upper | 0.574 (57.4%) | same | same | same; Audit ref: `ec:lovs:grepi:reporting-delay-update:2026-05-23` |
| Publication latency 50% lower | 2.64 days | same | same | brief and webpage visibility detail (when shown); Audit ref: `ec:lovs:grepi:reporting-delay-update:2026-05-23` |
| Publication latency 50% upper | 12.33 days | same | same | same; Audit ref: `ec:lovs:grepi:reporting-delay-update:2026-05-23` |
...
The brief Inferred-trajectory paragraph and the InferredTrajectory chart both surface the endpoint band 1249-1922 alongside the joint WHO-Imperial reference range 400-900 (Imperial 20 May 2026 update). The two bands are distinct: 1249-1922 is this snapshot's own deaths-back-projection at its laboratory-confirmed death input (183) and our re-grounded 7-day central; 400-900 is Imperial's published envelope over their two methods at their death input (131) and 14-day central.
...
277 corridors are emitted by the pipeline (the 31 INSP per-health-zone source zones that carry confirmed cases, each crossed with nine candidate target zones, minus two self-edges because goma-cod and beni-cod are each both a confirmed source zone and a candidate target). Across the current 277-corridor watchlist for the 2026-06-13 publication-state snapshot, the zone-attributed run's ascertainment-adjusted 50% ranges span 0.5-74.7% lower and 1.4-97.1% upper. Snapshot contract: 801 confirmed cases (782 DRC + 19 Uganda) are the headline aggregate (INSP SitRep #030 promotion, headline clock 2026-06-13); 688 confirmed cases are officially zone-attributed across 31 official source zones at the per-zone attribution clock (data_as_of 2026-06-13), leaving 113 confirmed cases unallocated to a public zone row. The headline clock and the per-zone attribution clock are deliberately distinct when a source publishes residual or non-health-zone rows and are not differenced as incidence. This is source-attribution lag, not missing cases. The source-load vector is the reviewed INSP per-health-zone Table 1; per-zone confirmed_deaths trail case attribution by roughly 1-3 weeks while the INRB clinical review queue closes. Audit refs: `ec:lovs:method:bdbv-zone-attributed-corridors:2026-05-22`; `ec:lovs:data:inrb-sitrep-030-visual-promotion:2026-06-13`. Numbers below are the active pre-committed calibration corridors carried forward from the immutable ledger: four from the 20 May top-ranked block and eight from the unpublished 21 May designed-sample block. They are not re-derived from future corridor rankings.
