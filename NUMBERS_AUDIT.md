# Numbers audit, 20 May 2026 snapshot
...
This file is the **single ground-truth registry** for the 20 May 2026 snapshot. If a downstream number does not appear here, or appears with a different attribution, that is a bug. Update this file in the same commit that updates the number.
...
| Deaths, May 20 | 144 | Archived consensus aggregator infobox | wikipedia-2026-ituri-epidemic-2026-05-20 | byte-archived | TrajectoryChart endpoint; AtAGlance deaths; refresh_pipeline.py reported_deaths.primary_value; Audit ref: `audit_gap:public-source-row` |
...
|---|---|---|---|---|
| Reporting completeness 50% lower | 0.592 (59%) | lovs/lovs_visibility.py nowcast() | reported_counts (above), affected_zones, snapshot.as_of | Bottom-line aside; AtAGlance visibility row; InferredTrajectory band; brief Ascertainment section "59 to 63 percent"; README headline finding; Audit ref: `ec:lovs:module-c:reporting-delay-priors:2026-05-20` |
| Reporting completeness 50% upper | 0.628 (63%) | same | same | same; Audit ref: `ec:lovs:module-c:reporting-delay-priors:2026-05-20` |
| Publication latency 50% lower | 1.1 days | same | same | brief and webpage visibility detail (when shown); Audit ref: `ec:lovs:module-c:reporting-delay-priors:2026-05-20` |
| Publication latency 50% upper | 6.7 days | same | same | same; Audit ref: `ec:lovs:module-c:reporting-delay-priors:2026-05-20` |
| Confirmation backlog 50% lower | 427 | same | same | snapshot JSON, not surfaced as headline number; Audit ref: `ec:lovs:module-c:reporting-delay-priors:2026-05-20` |
| Confirmation backlog 50% upper | 558 | same | same | same; Audit ref: `ec:lovs:module-c:reporting-delay-priors:2026-05-20` |
...
| P(exactly 4) | 0.020 | same | same | same; Audit refs: `ec:lovs:module-d:bdbv-r-prior-gamma:2026-05-20`, `ec:lovs:module-b:detection-depth-priors:2026-05-21` |
| P(exactly 5) | 0.053 | same | same | same; Audit refs: `ec:lovs:module-d:bdbv-r-prior-gamma:2026-05-20`, `ec:lovs:module-b:detection-depth-priors:2026-05-21` |
| P(>= 6, censored upper bin) | 0.927 | same | same | same; Audit refs: `ec:lovs:module-d:bdbv-r-prior-gamma:2026-05-20`, `ec:lovs:module-b:detection-depth-priors:2026-05-21` |
...
|---|---|---|---|---|
| May 15 | 4 | round(4/0.628) = 6 | round(4/0.592) = 7 | confirmed / completeness 50% endpoints; Audit ref: `ec:lovs:module-c:reporting-delay-priors:2026-05-20` |
| May 17 | 10 | round(10/0.628) = 16 | round(10/0.592) = 17 | same; Audit ref: `ec:lovs:module-c:reporting-delay-priors:2026-05-20` |
| May 19 | 30 | round(30/0.628) = 48 | round(30/0.592) = 51 | same; Audit ref: `ec:lovs:module-c:reporting-delay-priors:2026-05-20` |
| May 20 | 53 | round(53/0.628) = 84 | round(53/0.592) = 90 | same; Audit ref: `ec:lovs:module-c:reporting-delay-priors:2026-05-20` |
...
18 corridors are emitted by the pipeline (three Ituri source zones into six candidate target zones, after the 21 May 2026 revision added Arua and Nebbi to the candidate-target watch set). Across all 18, the current aggregate-input run's ascertainment-adjusted 50% upper bounds span **49.5 to 53.3 percent**, and the lower bounds span **20.9 to 24.7 percent**. Numbers below are the four pre-committed calibration corridors carried forward unchanged from the immutable ledger, not re-derived from the updated corridor ranking.
...
Statements are pre-committed; they resolve on **19 June 2026, 23:59:59 UTC** against publicly available DRC MoH, Uganda MoH, WHO, and Africa CDC reports.
...
`data/evidence-chains.json` is the structured companion to this narrative file. It encodes eleven audit claims as `chain_id` records with a `verdict` field (`corrected`, `derived_supported`, `needs_primary_source`, `unsupported_attribution`, `pending`) and the source chain that backs the verdict. `lovs/lovs_evidence.py` validates the registry on every run; `python3 -m lovs.lovs_evidence` reports the current counts.

Current state: 11 chains validate as `corrected: 3, derived_supported: 3, needs_primary_source: 2, unsupported_attribution: 2, pending: 1`. The three outstanding evidentiary gaps most relevant to this narrative are tracked there and surfaced here:
