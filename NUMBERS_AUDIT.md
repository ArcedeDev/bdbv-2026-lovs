...
**Scope note (current cumulative contract).** The rules and per-source rows in this section describe the historical 15-to-25 May reporting series, when suspected was still carried as a published cumulative tier. As of the 2026-07-08 SitRep #055 promotion, the only cumulative epidemiological metrics are laboratory-confirmed cases (1812 confirmed cases: 1792 DRC + 20 Uganda) and confirmed deaths (627: 625 DRC + 2 Uganda). The cumulative suspected tier and the confirmed-and-suspected composite are paused and archived from the headline surface (retained as dated provenance, and reactivatable in a future snapshot). Suspected cases now appear only as point-in-time operational caseloads such as suspected cases in isolation on a separate non-cumulative axis, and the per-source suspected rows below are retained as archival provenance of what each publisher reported on its own date, never summed into confirmed. SitRep #055 republishes a fresh patient-movement table (Tableau 7: 764 in isolation, split 236 confirmed / 528 suspected) but does not publish the under-investigation stock or the total active suspected queue, so those values are not fabricated. The present-tense "current LOVS contract" phrasing in the rules below is preserved as the historical record for that series.
...
331 corridors are emitted by the pipeline (the 37 INSP per-health-zone source zones that carry confirmed cases, each crossed with nine candidate target zones, minus two self-edges because goma-cod and beni-cod are each both a confirmed source zone and a candidate target). Across the current 331-corridor watchlist for the 2026-07-08 publication-state snapshot, the zone-attributed run's ascertainment-adjusted 50% ranges span 0.5-95.2% lower and 1.5-100.0% upper. Snapshot contract: 1812 confirmed cases (1792 DRC + 20 Uganda) are the headline aggregate (INSP SitRep #055 promotion, headline clock 2026-07-08); 1772 confirmed cases are officially zone-attributed across 37 official source zones at the per-zone attribution clock (data_as_of 2026-07-08), leaving 40 confirmed cases unallocated to a public zone row. The headline clock and the per-zone attribution clock are deliberately distinct when a source publishes residual or non-health-zone rows and are not differenced as incidence. This is source-attribution lag, not missing cases. The source-load vector is the coherent reviewed INSP per-health-zone Table 2 zone list; no newly named zone this cycle, and 17 Ituri confirmed cases remain in the explicit residual awaiting health-zone identification (plus 3 Nord-Kivu confirmed cases reported at province level but not ventilated to a named zone). Audit refs: `ec:lovs:method:bdbv-zone-attributed-corridors:2026-05-22`; `ec:lovs:data:inrb-sitrep-055-visual-promotion:2026-07-08`. Numbers below are the active pre-committed calibration corridors carried forward from the immutable ledger: four from the 20 May top-ranked block and eight from the unpublished 21 May designed-sample block. They are not re-derived from future corridor rankings.
