...
**Scope note (current cumulative contract).** The rules and per-source rows in this section describe the historical 15-to-25 May reporting series, when suspected was still carried as a published cumulative tier. As of the 2026-07-27 SitRep #074 promotion, the only cumulative epidemiological metrics are laboratory-confirmed cases (3380: 3360 DRC + 20 Uganda) and confirmed deaths (1489: 1487 DRC + 2 Uganda). Confirmed cases close 3262 + 98 = 3360. Deaths close 1437 + 50 = 1487, but the source declares +11 Nord-Kivu deaths from zone harmonization, so +50 is a net notification/reclassification value rather than pure incident mortality. The cumulative suspected tier and confirmed-and-suspected composite remain archived from the headline. Suspected cases appear only as point-in-time operational caseloads on a separate axis. SitRep #074 reports 733 patients in isolation at 77.3% occupancy; 719 J-1 + 140 admissions - 126 exits = 733 and 313 confirmed + 420 suspected = 733. The under-investigation stock and total active suspected queue are not fabricated.
...
The 430-corridor watchlist is emitted from the INSP per-health-zone surface: 48 official source zones crossed with nine candidate targets, minus the goma-cod and beni-cod self-edges. The roster is unchanged at SitRep #074. For the 2026-07-27 snapshot, adjusted 50% ranges span 0.5-99.4% lower and 1.5-100.0% upper; bunia -> arua-uga ranks first at 98.5-100.0%. The country headline is 3380 confirmed cases (3360 DRC + 20 Uganda); 3360 confirmed cases are officially zone-attributed, while the 20 confirmed cases remain explicit Uganda source-attribution lag. The detailed rows close to 3360/1487. The death movement contains a declared +11 Nord-Kivu harmonization and is not interpreted as pure incident mortality. Audit refs: `ec:lovs:method:bdbv-zone-attributed-corridors:2026-05-22`; `ec:lovs:data:inrb-sitrep-074-visual-promotion:2026-07-27`. Numbers below remain the pre-committed calibration corridors and are not re-derived from future rankings.
