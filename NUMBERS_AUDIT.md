...
152 corridors are emitted by the pipeline (the 22 INSP per-zone source zones that carry confirmed cases, each crossed with seven candidate target zones, minus two self-edges because goma-cod and beni-cod are each both a confirmed source zone and a candidate target). Across the current 152-corridor watchlist for the 2026-06-02 publication-state snapshot (corridor source-load on the INRB-UMIE/INSP per-health-zone series), the zone-attributed run's ascertainment-adjusted 50% ranges span 0.5-40.2% lower and 1.5-75.1% upper. Snapshot contract: 370 confirmed cases are the headline aggregate (INRB SitRep #019 promotion, headline clock 2026-06-02); 243 confirmed cases are officially zone-attributed across 25 official source zones at the per-zone attribution clock (data_as_of 2026-05-29), leaving 127 confirmed cases unallocated to a public zone row. The headline clock (2026-06-02) and the per-zone attribution clock (2026-05-29) are deliberately distinct and are not differenced as incidence. This is source-attribution lag, not missing cases. The source-load vector is the INRB-UMIE/INSP per-health-zone series (build-2026-06-01-b4cafc9, data as of 2026-05-29); per-zone confirmed_deaths trail case attribution by roughly 1-3 weeks while the INRB clinical review queue closes. Audit refs: `ec:lovs:method:bdbv-zone-attributed-corridors:2026-05-22`; `ec:lovs:data:inrb-sitrep-019-visual-promotion:2026-06-02`. Numbers below are the active pre-committed calibration corridors carried forward from the immutable ledger: four from the 20 May top-ranked block and eight from the unpublished 21 May designed-sample block. They are not re-derived from future corridor rankings.
