# Numbers audit, snapshot series through 25 May 2026
...
This file is the **single ground-truth registry** for the dated snapshot series through 25 May 2026. If a downstream number does not appear here, or appears with a different attribution, that is a bug. Update this file in the same commit that updates the number.
...
**Scope note (current cumulative contract).** The rules and per-source rows in this section describe the historical 15-to-25 May reporting series, when suspected was still carried as a published cumulative tier. As of the 2026-06-01 deaths-split and the 2026-06-02 suspected pause and archive, the only cumulative epidemiological metrics are laboratory-confirmed cases (328 = 321 DRC + 7 Uganda) and confirmed deaths (49 = 48 + 1). The cumulative suspected tier and the confirmed-and-suspected composite are paused and archived from the headline surface (retained as dated provenance, and reactivatable in a future snapshot). Suspected cases now appear only as a point-in-time operational caseload (under investigation and in isolation) on a separate non-cumulative axis, and the per-source suspected rows below are retained as archival provenance of what each publisher reported on its own date, never summed into confirmed. The present-tense "current LOVS contract" phrasing in the rules below is preserved as the historical record for that series.
...
| May 31 | 49 (lab-confirmed) | round(49 * 2.731 / 0.40) = 335 | round(49 * 2.731 / 0.26) = 515 | current endpoint (laboratory-confirmed deaths, our re-grounded 7-day central; suspected deaths never summed); Audit ref: `ec:lovs:website:cfr-band-correction:2026-05-20`; `ec:lovs:method:death-back-projection:2026-05-21`; `ec:lovs:data:inrb-sitrep-017-queue-drawdown:2026-05-31` |
...
The brief Inferred-trajectory paragraph and the InferredTrajectory chart both surface the endpoint band 335-515 alongside the joint WHO-Imperial reference range 400-900 (Imperial 20 May 2026 update). The two bands are distinct: 335-515 is this snapshot's own deaths-back-projection at its laboratory-confirmed death input (49) and our re-grounded 7-day central; 400-900 is Imperial's published envelope over their two methods at their death input (131) and 14-day central.
...
152 corridors are emitted by the pipeline (the 22 INSP per-zone source zones that carry confirmed cases, each crossed with seven candidate target zones, minus two self-edges because goma-cod and beni-cod are each both a confirmed source zone and a candidate target). Across the current 152-corridor watchlist for the 2026-05-31 publication-state snapshot (corridor source-load on the INRB-UMIE/INSP per-health-zone series), the zone-attributed run's ascertainment-adjusted 50% ranges are listed in the corridor table below. Snapshot contract: 328 confirmed cases are the headline aggregate (321 DRC + 7 Uganda per INRB SitRep #017, headline clock 2026-05-31); 243 confirmed cases are officially zone-attributed across 22 INSP per-zone source zones at the per-zone attribution clock (data_as_of 2026-05-29), against a zone-clock national of 263 (a 20-case zone-clock residual not yet zone-attributed). The headline clock (2026-05-31) and the per-zone attribution clock (2026-05-29) are deliberately distinct and are not differenced against each other. This is source-attribution lag, not missing cases. The source-load vector is the INRB-UMIE/INSP per-health-zone series (build-2026-06-01-b4cafc9, data as of 2026-05-29); per-zone confirmed_deaths trail case attribution by roughly 1-3 weeks while the INRB clinical review queue closes. Audit refs: `ec:lovs:method:bdbv-zone-attributed-corridors:2026-05-22`; `ec:lovs:data:inrb-sitrep-017-queue-drawdown:2026-05-31`. Numbers below are the active pre-committed calibration corridors carried forward from the immutable ledger: four from the 20 May top-ranked block and eight from the unpublished 21 May designed-sample block. They are not re-derived from future corridor rankings.
