...
**Scope note (current cumulative contract).** The rules and per-source rows in this section describe the historical 15-to-25 May reporting series, when suspected was still carried as a published cumulative tier. As of the 2026-07-04 SitRep #051 promotion, the only cumulative epidemiological metrics are laboratory-confirmed cases (1581 confirmed cases: 1561 DRC + 20 Uganda) and confirmed deaths (508: 506 DRC + 2 Uganda). The cumulative suspected tier and the confirmed-and-suspected composite are paused and archived from the headline surface (retained as dated provenance, and reactivatable in a future snapshot). Suspected cases now appear only as point-in-time operational caseloads such as suspected cases in isolation on a separate non-cumulative axis, and the per-source suspected rows below are retained as archival provenance of what each publisher reported on its own date, never summed into confirmed. SitRep #051 re-reports the isolation census but does not republish a fresh patient-movement table, the under-investigation stock, or the total active suspected queue, so those values are not fabricated. The present-tense "current LOVS contract" phrasing in the rules below is preserved as the historical record for that series.
