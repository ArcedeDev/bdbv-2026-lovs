...
Contract partition: 4747 headline confirmed, 4727 assigned across 55 official source zones, and 20 confirmed cases held as source-attribution lag.

**Scope note (current cumulative contract).** The rules and per-source rows in this section describe the historical 15-to-25 May reporting series, when suspected was still carried as a published cumulative tier. As of the 2026-08-13 SitRep #091 promotion, the only cumulative epidemiological metrics are laboratory-confirmed cases (4747: 4727 DRC + 20 Uganda), confirmed deaths (2216: 2214 DRC + 2 Uganda), and recovered (987: 976 DRC + 11 Uganda). SitRep #091's reviewed INSP per-zone table and province table close exactly to the DRC headline, attributing 4727 confirmed cases across 55 official source zones. The resulting 20-case attribution gap is exactly the separately sourced Uganda anchor and is not redistributed over DRC zones. SitRep #091 reports 749 people in isolation/CTEs without a national confirmed-versus-suspected split, so all 749 remain unclassified by case status; Ituri's 528/985 (53.6%) occupancy is provincial, not national capacity. Contact follow-up is 18113/21926 (82.6%); the lower rate versus SitRep #090 is retained with its larger denominator rather than interpreted as a capacity collapse. The cumulative suspected tier and confirmed-and-suspected composite remain archived from the headline, and missing fields are not fabricated.
...
The 493-corridor watchlist is emitted from the latest reviewed INSP per-health-zone surface: 55 official SitRep #091 source zones crossed with nine candidate targets, minus the goma-cod and beni-cod self-edges. For the 2026-08-14 publication-day snapshot, adjusted 50% ranges span 0.5-99.9% lower and 1.4-100.0% upper. The country headline is 4747 confirmed cases (4727 DRC + 20 Uganda), while 4727 confirmed cases are officially zone-attributed on the 2026-08-13 source clock; the residual 20 confirmed cases are exactly the separately sourced Uganda anchor. SitRep #091 republishes the full per-health-zone table after the compact #084/#085 editions and the #086/#087 LinkedIn image packets carried none, so DRC zone attribution is current with the DRC headline; Buta and the Tshopo health zone are served as their own GRID3 geometries rather than shaded onto neighbours. Audit refs: `ec:lovs:method:bdbv-zone-attributed-corridors:2026-05-22`; `ec:lovs:data:inrb-sitrep-090-visual-promotion:2026-08-12`; `ec:lovs:data:inrb-sitrep-091-visual-promotion:2026-08-13`. Numbers below remain the pre-committed calibration corridors and are not re-derived from future rankings.
