...
Contract partition: 4229 headline confirmed, 4053 assigned across 53 official source zones, and 176 confirmed cases held as source-attribution lag.

**Scope note (current cumulative contract).** The rules and per-source rows in this section describe the historical 15-to-25 May reporting series, when suspected was still carried as a published cumulative tier. As of the 2026-08-07 compact SitRep #085 promotion, the only cumulative epidemiological metrics are laboratory-confirmed cases (4229: 4209 DRC + 20 Uganda), confirmed deaths (1918: 1916 DRC + 2 Uganda), and recovered (839: 828 DRC + 11 Uganda). SitRep #085's five province rows close exactly at 4209 confirmed, 1916 deaths, and +89 confirmations; the 29-death daily movement reconciles the cumulative and province deltas as 21 newly confirmed deaths plus 8 deaths in CTEs. The compact edition publishes no health-zone case/death rows. The latest reviewed spatial allocation therefore remains SitRep #083 dated 2026-08-05: 4053 confirmed across 53 zones. The resulting 176-case attribution gap comprises 156 later DRC confirmations plus the 20-case Uganda anchor and is not redistributed. SitRep #085 reports 595 people in isolation/CTEs without a national confirmed-versus-suspected split, so all 595 remain unclassified by case status; Ituri's 595/860 (69.2%) occupancy is provincial, not national capacity. Contact follow-up is 14923/17896 (83.4%); the lower denominator versus SitRep #083 is retained as a possible list revision rather than interpreted as incidence. The cumulative suspected tier and confirmed-and-suspected composite remain archived from the headline, and missing fields are not fabricated.
...
The deaths-back-projection follows Imperial College MRC GIDA's Method 2 (18 May 2026 report, carried forward unchanged in the 20 May 2026 update): `total_cases = deaths * (1 + r/beta)^alpha / CFR`, with `r = ln(2)/tau_2`. Parameters: `tau_2 = 7 days` (our re-grounded central from this outbreak's confirmed-case series, 2026-06-01; Imperial's borrowed 14-day central is retained only to reproduce their published 400-900 reference), Rosello et al. 2015 eLife BDBV gamma `alpha = 4.42`, `beta = 0.388/day`, CFR scenario set `{0.26, 0.33, 0.40}` with central `0.33`, from the US CDC outbreak history aggregate (55 deaths across 169 cases = 32.5 percent over the 2007-08 Uganda and 2012 DRC outbreaks; the 0.26 and 0.40 bounds are the Wilson 95% CI [25.9 percent, 39.9 percent] of that proportion). This matches the Imperial 20 May 2026 update, which corrected the 18 May 24/30/40 set to 26/33/40. **tau_2 IS NO LONGER A CONSTANT.** It is re-estimated every cycle from this outbreak's own confirmed-case series over a trailing incidence window, with restated boundaries corrected before differencing, and the published band is built from that floated value. At SitRep 82 the floated central is `tau_2 = 58.1 days` (slow_growth). Because the confirmed series is ascertainment-confounded this cycle, that value is published as an UPPER BOUND on doubling and no confidence interval is issued: the dominant error is a one-sided bias, not counting error, so the band it drives is a FLOOR rather than a symmetric range. The central growth correction is `(1 + (ln2/58.1)/0.388)^4.42 = 1.14323`. The `tau_2 = 7 days` figure quoted above was the static central re-grounded on 2026-06-01; it is retained ONLY as the fallback used when no value can be floated, and as the correction under which the historical rows below were computed. Its 7-day-era correction was `(1 + (ln2/7)/0.388)^4.42 = 2.731`, so bands computed at 7 days are not comparable to bands computed today. Imperial's reference tau_2=14 gives `(1 + (ln2/14)/0.388)^4.42 = 1.700` and stays frozen for reproduction only. The Method-2 death input is laboratory-confirmed deaths only (1803 country-scope at the SitRep #082 2026-08-04 endpoint, giving a floor band of 5153 to 7928 with central 6246); the suspected-inclusive composite (247) used before the 2026-06-01 deaths-split is superseded.
...
| June 27 | 362 (lab-confirmed) | round(362 * 2.731 / 0.40) = 2472 | round(362 * 2.731 / 0.26) = 3802 | 2026-06-27 endpoint at the then-current 7d-era correction (laboratory-confirmed deaths, our re-grounded 7-day central; suspected deaths never summed; deaths input is the SitRep #044 June 27 country-scope headline 360 DRC + 2 Uganda; SitRep #043 was not published); Audit ref: `ec:lovs:website:cfr-band-correction:2026-05-20`; `ec:lovs:method:death-back-projection:2026-05-21`; `ec:lovs:data:inrb-sitrep-044-visual-promotion:2026-06-27` |

**Basis note for the table above.** Every row is a HISTORICAL record of what was published at that endpoint, computed at the growth correction in force on that date (1.700/1.7003 for the Imperial 14-day reference rows, 2.731 for the 7-day-era rows). They are deliberately NOT recomputed at the current floated correction: recomputing them would destroy the audit trail of what each dated artifact actually said. Do not compare a row's band to a band published today without first noting that the correction changed from 2.731 to 1.597.
...
The 475-corridor watchlist is emitted from the latest reviewed INSP per-health-zone surface: 53 official SitRep #083 source zones crossed with nine candidate targets, minus the goma-cod and beni-cod self-edges. For the 2026-08-07 snapshot, adjusted 50% ranges span 0.5-99.8% lower and 1.5-100.0% upper. The country headline is 4229 confirmed cases (4209 DRC + 20 Uganda), while 4053 confirmed cases remain officially zone-attributed on the 2026-08-05 clock; the 176-case attribution gap is 156 later DRC confirmations plus the Uganda anchor. Compact SitReps #084/#085 add no zone rows, so no zone enters or leaves the spatial source surface this cycle. Audit refs: `ec:lovs:method:bdbv-zone-attributed-corridors:2026-05-22`; `ec:lovs:data:inrb-sitrep-083-visual-promotion:2026-08-05`; `ec:lovs:data:inrb-sitrep-085-compact-promotion:2026-08-07`. Numbers below remain the pre-committed calibration corridors and are not re-derived from future rankings.
