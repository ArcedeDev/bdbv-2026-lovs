...
**Scope note (current cumulative contract).** The rules and per-source rows in this section describe the historical 15-to-25 May reporting series, when suspected was still carried as a published cumulative tier. As of the 2026-08-01 SitRep #079 promotion, the only cumulative epidemiological metrics are laboratory-confirmed cases (3768: 3748 DRC + 20 Uganda) and confirmed deaths (1659: 1657 DRC + 2 Uganda). All 50 printed DRC rows close to 3748/1657, but official coverage remains 49 zones because Wanie-Rukula's one-case row is pending harmonization; the official corridor source-load is therefore 3747 and the attribution gap is 21. The 24-hour rows close at +74 confirmed and +36 deaths. The cumulative suspected tier and confirmed-and-suspected composite remain archived from the headline. Suspected cases appear only as point-in-time operational caseloads on a separate axis. SitRep #079 reports 690 patients in isolation at 65.7% occupancy; the provincial and 333-confirmed + 357-suspected decompositions close. Haut-Uele patient movement leaves a 14-patient mismatch. The under-investigation stock and total active suspected queue are not fabricated.
...
The 439-corridor watchlist is emitted from the INSP per-health-zone surface: 49 official source zones crossed with nine candidate targets, minus the goma-cod and beni-cod self-edges. Wanie-Rukula remains pending harmonization and is not added. For the 2026-08-01 snapshot, adjusted 50% ranges span 0.5-99.7% lower and 1.5-100.0% upper; bunia -> kisangani-cod ranks first at 99.7-100.0%. The country headline is 3768 confirmed cases (3748 DRC + 20 Uganda); 3747 confirmed cases are officially zone-attributed, while 21 confirmed cases remain an explicit source-attribution lag comprising 20 Uganda cases and the pending one-case Wanie-Rukula row. Audit refs: `ec:lovs:method:bdbv-zone-attributed-corridors:2026-05-22`; `ec:lovs:data:inrb-sitrep-079-visual-promotion:2026-08-01`. Numbers below remain the pre-committed calibration corridors and are not re-derived from future rankings.
