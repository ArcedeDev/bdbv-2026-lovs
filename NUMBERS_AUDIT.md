...
**Scope note (current cumulative contract).** The rules and per-source rows in this section describe the historical 15-to-25 May reporting series, when suspected was still carried as a published cumulative tier. As of the 2026-06-15 SitRep #032 promotion, the only cumulative epidemiological metrics are laboratory-confirmed cases (856 confirmed cases: 837 DRC + 19 Uganda) and confirmed deaths (198: 196 DRC + 2 Uganda). The cumulative suspected tier and the confirmed-and-suspected composite are paused and archived from the headline surface (retained as dated provenance, and reactivatable in a future snapshot). Suspected cases now appear only as point-in-time operational caseloads such as suspected cases in isolation on a separate non-cumulative axis, and the per-source suspected rows below are retained as archival provenance of what each publisher reported on its own date, never summed into confirmed. SitRep #032 does not republish the under-investigation stock or total active suspected queue, so those values are not fabricated. The present-tense "current LOVS contract" phrasing in the rules below is preserved as the historical record for that series.
...
The brief Inferred-trajectory paragraph and the InferredTrajectory chart both surface the endpoint band 1352-2080 alongside the joint WHO-Imperial reference range 400-900 (Imperial 20 May 2026 update). The two bands are distinct: 1352-2080 is this snapshot's own deaths-back-projection at its laboratory-confirmed death input (198) and our re-grounded 7-day central; 400-900 is Imperial's published envelope over their two methods at their death input (131) and 14-day central.
...
277 corridors are emitted by the pipeline (the 31 INSP per-health-zone source zones that carry confirmed cases, each crossed with nine candidate target zones, minus two self-edges because goma-cod and beni-cod are each both a confirmed source zone and a candidate target). Across the current 277-corridor watchlist for the 2026-06-15 publication-state snapshot, the zone-attributed run's ascertainment-adjusted 50% ranges span 0.5-75.4% lower and 1.5-97.9% upper. Snapshot contract: 856 confirmed cases (837 DRC + 19 Uganda) are the headline aggregate (INSP SitRep #032 promotion, headline clock 2026-06-15); 743 confirmed cases are officially zone-attributed across 31 official source zones at the per-zone attribution clock (data_as_of 2026-06-15), leaving 113 confirmed cases unallocated to a public zone row. The headline clock and the per-zone attribution clock are deliberately distinct when a source publishes residual or non-health-zone rows and are not differenced as incidence. This is source-attribution lag, not missing cases. The source-load vector is the reviewed INSP per-health-zone Table 2; per-zone confirmed_deaths trail case attribution by roughly 1-3 weeks while the INRB clinical review queue closes. Audit refs: `ec:lovs:method:bdbv-zone-attributed-corridors:2026-05-22`; `ec:lovs:data:inrb-sitrep-032-visual-promotion:2026-06-15`. Numbers below are the active pre-committed calibration corridors carried forward from the immutable ledger: four from the 20 May top-ranked block and eight from the unpublished 21 May designed-sample block. They are not re-derived from future corridor rankings.
