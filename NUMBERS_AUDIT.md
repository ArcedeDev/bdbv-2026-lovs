...
**Scope note (current cumulative contract).** The rules and per-source rows in this section describe the historical 15-to-25 May reporting series, when suspected was still carried as a published cumulative tier. As of the 2026-07-30 SitRep #077 promotion, the only cumulative epidemiological metrics are laboratory-confirmed cases (3625: 3605 DRC + 20 Uganda) and confirmed deaths (1589: 1587 DRC + 2 Uganda). The 49 detailed DRC zone rows close exactly to the national totals; the source tables report +73 cases in 24 hours while prose reports +77, so LOVS promotes the reconciled table value and preserves the conflict. The cumulative suspected tier and confirmed-and-suspected composite remain archived from the headline. Suspected cases appear only as point-in-time operational caseloads on a separate axis. SitRep #077 reports 748 patients in isolation at 71.3% occupancy; the provincial and 337-confirmed + 411-suspected decompositions close, while the movement identity yields 759 and is preserved as an 11-patient conflict. The under-investigation stock and total active suspected queue are not fabricated.
...
The 439-corridor watchlist is emitted from the INSP per-health-zone surface: 49 official source zones crossed with nine candidate targets, minus the goma-cod and beni-cod self-edges. Kabondo is newly integrated at SitRep #077; Wanie-Rukula remains pending harmonization and is not added. For the 2026-07-30 snapshot, adjusted 50% ranges span 0.5-99.6% lower and 1.5-100.0% upper; bunia -> kisangani-cod ranks first at 99.5-100.0%. The country headline is 3625 confirmed cases (3605 DRC + 20 Uganda); 3605 confirmed cases are officially zone-attributed, while the 20 confirmed cases remain explicit Uganda source-attribution lag. The detailed rows close to 3605/1587. Audit refs: `ec:lovs:method:bdbv-zone-attributed-corridors:2026-05-22`; `ec:lovs:data:inrb-sitrep-077-visual-promotion:2026-07-30`. Numbers below remain the pre-committed calibration corridors and are not re-derived from future rankings.
