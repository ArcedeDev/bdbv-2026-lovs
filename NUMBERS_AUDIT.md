...
| May 26 | 247 | round(247 * 1.7003 / 0.40) = 1050 | round(247 * 1.7003 / 0.26) = 1615 | current endpoint (snapshot primary; disclosed country-scope composition of 246 DRC suspected deaths from INRB/INSP/UMIE plus one Uganda confirmed death from ECDC/CDC); Audit ref: `ec:lovs:website:cfr-band-correction:2026-05-20`; `ec:lovs:method:death-back-projection:2026-05-21`; `ec:lovs:method:drc-moh-26-may-headline-promotion:2026-05-27` |
...
The brief Inferred-trajectory paragraph and the InferredTrajectory chart both surface the endpoint band 1050-1615 alongside the joint WHO-Imperial reference range 400-900 (Imperial 20 May 2026 update). The two bands are distinct: 1050-1615 is this snapshot's own deaths-back-projection at its disclosed death input (247, composed from 246 DRC suspected deaths plus one Uganda confirmed death); 400-900 is Imperial's published envelope over their two methods at their death input (131).
...
76 corridors are emitted by the pipeline (the 11 INSP per-zone source zones that carry confirmed cases, each crossed with seven candidate target zones, minus one self-edge because goma-cod is both a confirmed source zone and a candidate target; the seven monitored zones with zero confirmed cases generate no corridor, since a zero-confirmed zone has no observed transmission source to spill over). Across the current 76-corridor watchlist for the 28 May 2026 publication-state snapshot (corridor source-load re-based onto the INSP per-health-zone series, forward-only), the zone-attributed run's ascertainment-adjusted 50% range spans **0.7-24.9% lower bounds** and **1.9-51.9% upper bounds**. Snapshot contract: 128 confirmed cases are the headline aggregate (121 DRC + 7 Uganda per ECDC 27 May citing DRC MoH 26 May, with INRB build-2026-05-28 cross-corroborating the DRC component); 109 confirmed cases are officially zone-attributed across 18 INSP per-zone source zones (11 carrying confirmed cases); 19 confirmed cases remain unallocated headline context (12 DRC cases not yet zone-attributed plus Uganda's 7 cross-border cases) until an official zone table assigns them. This is source-attribution lag, not missing cases. The source-load vector is the INRB-UMIE/INSP per-health-zone series (build-2026-05-28-bb8b7d5, data as of 26 May 2026); per-zone confirmed_deaths trail case attribution by roughly 1-3 weeks while the INRB clinical review queue closes. Audit refs: `ec:lovs:method:bdbv-zone-attributed-corridors:2026-05-22`; `ec:lovs:method:drc-moh-26-may-headline-promotion:2026-05-27`. Numbers below are the active pre-committed calibration corridors carried forward from the immutable ledger: four from the 20 May top-ranked block and eight from the unpublished 21 May designed-sample block. They are not re-derived from future corridor rankings.
