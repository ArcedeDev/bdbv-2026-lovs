...
**Scope note (current cumulative contract).** The rules and per-source rows in this section describe the historical 15-to-25 May reporting series, when suspected was still carried as a published cumulative tier. As of the 2026-08-02 SitRep #080 promotion, the only cumulative epidemiological metrics are laboratory-confirmed cases (3822: 3802 DRC + 20 Uganda) and confirmed deaths (1709: 1707 DRC + 2 Uganda). All 51 printed DRC rows close exactly to 3802/1707 and official coverage expands to 51 zones as Lubero records its first confirmed case and Wanie-Rukula completes harmonization; the official corridor source-load is therefore 3802 and the attribution gap is 20, entirely the Uganda anchor. Nord-Kivu declares a retrospective harmonization, so the cumulative step of +54 confirmed and +50 deaths is a restatement; the notified 24-hour movement is +58 confirmed and +40 deaths. The cumulative suspected tier and confirmed-and-suspected composite remain archived from the headline. Suspected cases appear only as point-in-time operational caseloads on a separate axis. SitRep #080 reports 707 patients in isolation at 67.3% occupancy over 1050 reported beds; the provincial and 335-confirmed + 372-suspected decompositions close exactly. Haut-Uele patient movement leaves a 14-patient mismatch. The under-investigation stock and total active suspected queue are not fabricated.
...
The 457-corridor watchlist is emitted from the INSP per-health-zone surface: 51 official source zones crossed with nine candidate targets, minus the goma-cod and beni-cod self-edges. Lubero enters with its first confirmed case and Wanie-Rukula completes harmonization, taking the roster from 49 to 51. For the 2026-08-02 snapshot, adjusted 50% ranges span 0.5-99.7% lower and 1.5-100.0% upper; bunia -> kisangani-cod ranks first at 99.7-100.0%. The country headline is 3822 confirmed cases (3802 DRC + 20 Uganda); 3802 confirmed cases are officially zone-attributed, while 20 confirmed cases remain an explicit source-attribution lag comprising the Uganda anchor alone. Audit refs: `ec:lovs:method:bdbv-zone-attributed-corridors:2026-05-22`; `ec:lovs:data:inrb-sitrep-080-visual-promotion:2026-08-02`. Numbers below remain the pre-committed calibration corridors and are not re-derived from future rankings.
