...
76 corridors are emitted by the pipeline (11 official DRC source zones into seven candidate target zones after the 26 May 2026 promotion added Goma to the candidate-target watch set, minus one self-edge because Goma is both a source zone in the SitRep007 cumulative table and a target). Across the current 76-corridor watchlist for the 27 May 2026 publication-state snapshot, the corrected zone-attributed run's ascertainment-adjusted 50% range spans **0.6-22.2% lower bounds** and **1.8-49.4% upper bounds**. Snapshot contract: 128 confirmed cases are the headline aggregate (121 DRC + 7 Uganda per ECDC 27 May citing DRC MoH 26 May, with INRB build-2026-05-27 cross-corroborating the DRC component); 79 confirmed cases are officially zone-attributed across 11 DRC MoH source zones; 49 confirmed cases remain unallocated headline context until an official zone table assigns them. This is source-attribution lag, not missing cases. The source-load vector is the DRC MoH SitRep MVE N 007/MVB_17/2026 PDF cumulative Table IV: 79 confirmed DRC cases across Bambu, Bunia, Butembo, Goma, Katwa, Kilo/Kilo Mission, Miti-Murhesa, Mongbwalu, Nizi, Nyankunde, and Rwampara. Audit refs: `ec:lovs:method:bdbv-zone-attributed-corridors:2026-05-22`; `ec:lovs:method:drc-moh-26-may-headline-promotion:2026-05-27`. Numbers below are the active pre-committed calibration corridors carried forward from the immutable ledger: four from the 20 May top-ranked block and eight from the unpublished 21 May designed-sample block. They are not re-derived from future corridor rankings.
