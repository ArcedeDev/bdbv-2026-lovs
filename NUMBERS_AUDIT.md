# Numbers audit, snapshot series through 1 June 2026
...
This file is the **single ground-truth registry** for the dated snapshot series through 1 June 2026. If a downstream number does not appear here, or appears with a different attribution, that is a bug. Update this file in the same commit that updates the number.
...
**Scope note (current cumulative contract).** The rules and per-source rows in this section describe the historical 15-to-25 May reporting series, when suspected was still carried as a published cumulative tier. As of the 2026-06-02 SitRep #019 promotion, the only cumulative epidemiological metrics are laboratory-confirmed cases (378 confirmed cases (363 DRC + 15 Uganda; Uganda anchor refreshed from the stale ECDC 2026-05-27=7 to CDC 2026-06-02=15)) and confirmed deaths (63). The cumulative suspected tier and the confirmed-and-suspected composite are paused and archived from the headline surface (retained as dated provenance, and reactivatable in a future snapshot). Suspected cases now appear only as point-in-time operational caseloads such as suspected cases in isolation on a separate non-cumulative axis, and the per-source suspected rows below are retained as archival provenance of what each publisher reported on its own date, never summed into confirmed. SitRep #019 does not republish the under-investigation stock or total active suspected queue, so those values are not fabricated. The present-tense "current LOVS contract" phrasing in the rules below is preserved as the historical record for that series.
...
| Reporting completeness 50% upper | 0.459 (46%) | same | same | same; Audit ref: `ec:lovs:grepi:reporting-delay-update:2026-05-23` |
| Publication latency 50% lower | 3.14 days | same | same | brief and webpage visibility detail (when shown); Audit ref: `ec:lovs:grepi:reporting-delay-update:2026-05-23` |
| Publication latency 50% upper | 13.12 days | same | same | same; Audit ref: `ec:lovs:grepi:reporting-delay-update:2026-05-23` |
| Confirmation backlog 50% lower | 462 | same | same | snapshot JSON, not surfaced as headline number; Audit ref: `ec:lovs:grepi:reporting-delay-update:2026-05-23` |
| Confirmation backlog 50% upper | 720 | same | same | same; Audit ref: `ec:lovs:grepi:reporting-delay-update:2026-05-23` |
...
The brief Inferred-trajectory paragraph and the InferredTrajectory chart both surface the endpoint band 942-1450 alongside the joint WHO-Imperial reference range 400-900 (Imperial 20 May 2026 update). The two bands are distinct: 942-1450 is this snapshot's own deaths-back-projection at its laboratory-confirmed death input (138) and our re-grounded 7-day central; 400-900 is Imperial's published envelope over their two methods at their death input (131) and 14-day central.
...
152 corridors are emitted by the pipeline (the 22 INSP per-zone source zones that carry confirmed cases, each crossed with seven candidate target zones, minus two self-edges because goma-cod and beni-cod are each both a confirmed source zone and a candidate target). Across the current 152-corridor watchlist for the 2026-06-02 publication-state snapshot (corridor source-load on the INRB-UMIE/INSP per-health-zone series), the zone-attributed run's ascertainment-adjusted 50% ranges span 0.5-40.2% lower and 1.5-75.1% upper. Snapshot contract: 378 confirmed cases (363 DRC + 15 Uganda; Uganda anchor refreshed from the stale ECDC 2026-05-27=7 to CDC 2026-06-02=15) are the headline aggregate (INRB SitRep #019 promotion, headline clock 2026-06-02); 243 confirmed cases are officially zone-attributed across 25 official source zones at the per-zone attribution clock (data_as_of 2026-05-29), leaving 135 confirmed cases unallocated to a public zone row. The headline clock (2026-06-02) and the per-zone attribution clock (2026-05-29) are deliberately distinct and are not differenced as incidence. This is source-attribution lag, not missing cases. The source-load vector is the INRB-UMIE/INSP per-health-zone series (build-2026-06-01-b4cafc9, data as of 2026-05-29); per-zone confirmed_deaths trail case attribution by roughly 1-3 weeks while the INRB clinical review queue closes. Audit refs: `ec:lovs:method:bdbv-zone-attributed-corridors:2026-05-22`; `ec:lovs:data:inrb-sitrep-019-visual-promotion:2026-06-02`. Numbers below are the active pre-committed calibration corridors carried forward from the immutable ledger: four from the 20 May top-ranked block and eight from the unpublished 21 May designed-sample block. They are not re-derived from future corridor rankings.
