# Numbers audit, snapshot series through 11 June 2026
...
This file is the **single ground-truth registry** for the dated snapshot series through 11 June 2026. If a downstream number does not appear here, or appears with a different attribution, that is a bug. Update this file in the same commit that updates the number.
...
**Scope note (current cumulative contract).** The rules and per-source rows in this section describe the historical 15-to-25 May reporting series, when suspected was still carried as a published cumulative tier. As of the 2026-06-11 SitRep #028 promotion, the only cumulative epidemiological metrics are laboratory-confirmed cases (708 confirmed cases: 689 DRC + 19 Uganda) and confirmed deaths (141: 139 DRC + 2 Uganda). The cumulative suspected tier and the confirmed-and-suspected composite are paused and archived from the headline surface (retained as dated provenance, and reactivatable in a future snapshot). Suspected cases now appear only as point-in-time operational caseloads such as suspected cases in isolation on a separate non-cumulative axis, and the per-source suspected rows below are retained as archival provenance of what each publisher reported on its own date, never summed into confirmed. SitRep #028 does not republish the under-investigation stock or total active suspected queue, so those values are not fabricated. The present-tense "current LOVS contract" phrasing in the rules below is preserved as the historical record for that series.
...
| June 11 | 141 (lab-confirmed) | round(141 * 2.731 / 0.40) = 962 | round(141 * 2.731 / 0.26) = 1481 | current endpoint (laboratory-confirmed deaths, our re-grounded 7-day central; suspected deaths never summed; deaths input is the SitRep #028 June 11 headline); Audit ref: `ec:lovs:website:cfr-band-correction:2026-05-20`; `ec:lovs:method:death-back-projection:2026-05-21`; `ec:lovs:data:inrb-sitrep-028-visual-promotion:2026-06-11` |
...
The brief Inferred-trajectory paragraph and the InferredTrajectory chart both surface the endpoint band 962-1481 alongside the joint WHO-Imperial reference range 400-900 (Imperial 20 May 2026 update). The two bands are distinct: 962-1481 is this snapshot's own deaths-back-projection at its laboratory-confirmed death input (141) and our re-grounded 7-day central; 400-900 is Imperial's published envelope over their two methods at their death input (131) and 14-day central.
...
259 corridors are emitted by the pipeline (the 29 INSP per-health-zone source zones that carry confirmed cases, each crossed with nine candidate target zones, minus two self-edges because goma-cod and beni-cod are each both a confirmed source zone and a candidate target). Across the current 259-corridor watchlist for the 2026-06-11 publication-state snapshot, the zone-attributed run's ascertainment-adjusted 50% ranges span 0.5-71.8% lower and 1.5-96.1% upper. Snapshot contract: 708 confirmed cases (689 DRC + 19 Uganda) are the headline aggregate (INSP SitRep #028 promotion, headline clock 2026-06-11); 595 confirmed cases are officially zone-attributed across 29 official source zones at the per-zone attribution clock (data_as_of 2026-06-11), leaving 113 confirmed cases unallocated to a public zone row. The headline clock and the per-zone attribution clock are deliberately distinct when a source publishes residual or non-health-zone rows and are not differenced as incidence. This is source-attribution lag, not missing cases. The source-load vector is the reviewed INSP per-health-zone Table 1; per-zone confirmed_deaths trail case attribution by roughly 1-3 weeks while the INRB clinical review queue closes. Audit refs: `ec:lovs:method:bdbv-zone-attributed-corridors:2026-05-22`; `ec:lovs:data:inrb-sitrep-028-visual-promotion:2026-06-11`. Numbers below are the active pre-committed calibration corridors carried forward from the immutable ledger: four from the 20 May top-ranked block and eight from the unpublished 21 May designed-sample block. They are not re-derived from future corridor rankings.
