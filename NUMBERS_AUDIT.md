...
322 corridors are emitted by the pipeline (the 36 INSP per-health-zone source zones that carry confirmed cases, each crossed with nine candidate target zones, minus two self-edges because goma-cod and beni-cod are each both a confirmed source zone and a candidate target). Across the current 322-corridor watchlist for the 2026-06-30 publication-state snapshot, the zone-attributed run's ascertainment-adjusted 50% ranges span 0.5-92.1% lower and 1.4-99.9% upper. Snapshot contract: 1426 confirmed cases (1406 DRC + 20 Uganda) are the headline aggregate (INSP SitRep #047 promotion, headline clock 2026-06-30); 1389 confirmed cases are officially zone-attributed across 36 official source zones at the per-zone attribution clock (data_as_of 2026-06-30), leaving 37 confirmed cases unallocated to a public zone row. The headline clock and the per-zone attribution clock are deliberately distinct when a source publishes residual or non-health-zone rows and are not differenced as incidence. This is source-attribution lag, not missing cases. The source-load vector is the coherent reviewed INSP per-health-zone section 3.2 zone list; Lolwa (Ituri) is newly named this cycle, and 17 Ituri confirmed cases remain in the explicit residual awaiting health-zone identification. Audit refs: `ec:lovs:method:bdbv-zone-attributed-corridors:2026-05-22`; `ec:lovs:data:inrb-sitrep-047-visual-promotion:2026-06-30`. Numbers below are the active pre-committed calibration corridors carried forward from the immutable ledger: four from the 20 May top-ranked block and eight from the unpublished 21 May designed-sample block. They are not re-derived from future corridor rankings.
