...
**Scope note (current cumulative contract).** The rules and per-source rows in this section describe the historical 15-to-25 May reporting series, when suspected was still carried as a published cumulative tier. As of the 2026-07-31 SitRep #078 promotion, the only cumulative epidemiological metrics are laboratory-confirmed cases (3694: 3674 DRC + 20 Uganda) and confirmed deaths (1623: 1621 DRC + 2 Uganda). The 49 detailed DRC zone rows close exactly to the national totals, and the 24-hour rows close at +69 confirmed and +34 deaths. The cumulative suspected tier and confirmed-and-suspected composite remain archived from the headline. Suspected cases appear only as point-in-time operational caseloads on a separate axis. SitRep #078 reports 760 patients in isolation at 72.4% occupancy; the provincial and 348-confirmed + 412-suspected decompositions close. Haut-Uele admissions and exits are missing, and incorporating its observed net change into the published movement rows yields a one-patient mismatch. The under-investigation stock and total active suspected queue are not fabricated.
...
The 439-corridor watchlist is emitted from the INSP per-health-zone surface: 49 official source zones crossed with nine candidate targets, minus the goma-cod and beni-cod self-edges. Wanie-Rukula remains pending harmonization and is not added. For the 2026-07-31 snapshot, adjusted 50% ranges span 0.5-99.7% lower and 1.4-100.0% upper; bunia -> kampala-uga ranks first at 99.7-100.0%. The country headline is 3694 confirmed cases (3674 DRC + 20 Uganda); 3674 confirmed cases are officially zone-attributed, while the 20 confirmed cases remain explicit Uganda source-attribution lag. The detailed rows close to 3674/1621. Audit refs: `ec:lovs:method:bdbv-zone-attributed-corridors:2026-05-22`; `ec:lovs:data:inrb-sitrep-078-visual-promotion:2026-07-31`. Numbers below remain the pre-committed calibration corridors and are not re-derived from future rankings.
