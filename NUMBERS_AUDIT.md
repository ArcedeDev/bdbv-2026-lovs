...
**Scope note (current cumulative contract).** The rules and per-source rows in this section describe the historical 15-to-25 May reporting series, when suspected was still carried as a published cumulative tier. As of the 2026-08-04 SitRep #082 promotion, the only cumulative epidemiological metrics are laboratory-confirmed cases (3993: 3973 DRC + 20 Uganda) and confirmed deaths (1803: 1801 DRC + 2 Uganda). All 51 printed DRC rows close exactly to 3973 confirmed cases and 1801 deaths, and every province total matches Table 1: the two-death North Kivu residual carried in SitRep #081 is resorbed in this edition. The official corridor source-load is 3973 and the 20-case attribution gap is entirely the Uganda anchor. The notified 24-hour movement is +99 confirmed and +52 deaths, with no new zone; the national death cumulative advances only 50 because it also absorbs the two-unit correction, so row-to-row movement (+52) is the day's true figure. The cumulative suspected tier and confirmed-and-suspected composite remain archived from the headline. Suspected cases appear only as point-in-time operational caseloads on a separate axis. SitRep #082 reports 674 patients in isolation, classifying 311 as confirmed and 343 as suspected; Tshopo's 3 and Sud-Kivu's 17 are not split by status and remain unclassified, and Haut-Uele reports no census at all so it is absent from the 674. The under-investigation stock and missing status assignments are not fabricated.
...
The 457-corridor watchlist is emitted from the INSP per-health-zone surface: 51 official source zones crossed with nine candidate targets, minus the goma-cod and beni-cod self-edges. For the 2026-08-04 snapshot, adjusted 50% ranges span 0.5-99.7% lower and 1.4-100.0% upper; bunia -> kampala-uga ranks first at 99.7-100.0%. The country headline is 3993 confirmed cases (3973 DRC + 20 Uganda); 3973 confirmed cases are officially zone-attributed, while 20 confirmed cases remain an explicit source-attribution lag comprising the Uganda anchor alone. No zone enters or leaves this cycle. Audit refs: `ec:lovs:method:bdbv-zone-attributed-corridors:2026-05-22`; `ec:lovs:data:inrb-sitrep-082-visual-promotion:2026-08-04`. Numbers below remain the pre-committed calibration corridors and are not re-derived from future rankings.
