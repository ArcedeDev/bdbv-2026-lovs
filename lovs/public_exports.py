...
- Quantitative model outputs, private calibration workbench details, private-data adaptation, and source collection mechanics are outside the public export contract. Calibration probabilities, scoring rules, and resolvers are published; see `CALIBRATION_LEDGER_PUBLIC.md`.
...
- **Two May source values corrected from their archived pages.**
  ...
  - The WHO DON parser no longer reads "N deaths among confirmed cases" as a
    ...
    fallback read counts in digits, with or without thousands separators, or in words up
    to ninety-nine, and refuse a larger number written in words.
