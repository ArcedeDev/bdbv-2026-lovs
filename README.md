...
The current public artifact is an 11 June 2026 analytic snapshot: headline counts are dated 11 June 2026 (reviewed INSP SitRep #028 promotion) and DRC INSP per-health-zone attribution is dated 11 June 2026 from the reviewed INSP SitRep Table 1. Based on the public source package in this repo:

- **708 confirmed cases** as the current country-scope confirmed endpoint in [`data/public_snapshot.json`](data/public_snapshot.json). Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **141 confirmed deaths** as the cumulative confirmed-death endpoint.
- **595 confirmed cases officially zone-attributed across 29 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv) are the primary descriptive spread signal. The 708-case headline leaves **113 confirmed cases unallocated** to a public health-zone row: 94 DRC confirmed cases are explicitly unventilated by health zone in SitRep #028, and 19 Uganda confirmed cases have public country/district reporting but no promoted Uganda health-zone source-load.
- **177 suspected cases in isolation at 2026-06-11** in the separate `operational_status` block. SitRep #028 does not republish the under-investigation stock or total active suspected queue; those values are not fabricated. This is a point-in-time operational caseload, national-only, not a cumulative case count, and never added to confirmed.
