...
The current public artifact is a 12 July 2026 analytic snapshot: headline counts are dated 12 July 2026 (reviewed INSP SitRep #059 promotion) and DRC INSP per-health-zone attribution is dated 10 July 2026 (last consolidated distribution) from the reviewed INSP SitRep Table 2 zone list. Based on the public source package in this repo:

- **1983 confirmed cases** as the current country-scope confirmed endpoint in [`data/public_snapshot.json`](data/public_snapshot.json), composed of **1963 confirmed cases** across the five affected DRC provinces (Ituri, Nord-Kivu, Sud-Kivu, Haut-Uele, Tshopo) plus the 20-case Uganda anchor. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **720 confirmed deaths** as the cumulative confirmed-death endpoint.
- **1876 confirmed cases officially zone-attributed across 40 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv) are the primary descriptive spread signal. The 1983-case headline leaves **107 confirmed cases unallocated** to a public health-zone row: 17 DRC Ituri confirmed cases are explicitly awaiting health-zone identification, a 70-case residual reflects Ituri's per-zone distribution being carried at the 10 July consolidated cut while the province total is the 12 July cumulative (DHIS2 reclassification lag), and 20 Uganda confirmed cases have public country/district reporting but no promoted Uganda health-zone source-load. No new health zone was notified this cycle; the outbreak holds at 42 health zones.
- **490 suspected cases in isolation at 2026-07-12** in the separate `operational_status` block. SitRep #059 publishes a fresh Tableau 5 patient-movement table: 736 in isolation at 88.3% occupancy, split 246 confirmed / 490 suspected; the under-investigation stock and total active suspected queue are not republished, and those values are not fabricated. This is a point-in-time operational caseload, national-only, not a cumulative case count, and never added to confirmed.
...
For the current contract, the 107 confirmed cases not yet officially zone-attributed are treated as source-attribution lag, not as hidden model redistribution. Confirmed-death attribution retains the 1-3 week trailing-note disclosure; SitRep #059 republishes per-zone confirmed deaths for the coherent 42-zone vector, with the 17 Ituri unventilated cases plus the 10-July distribution-lag residual held in the explicit unallocated residual. The corridor surface is a descriptive 358-corridor watchlist over the reviewed source-load vector; its adjusted 50% interval ranges span 0.5-96.4% lower and 1.5-100.0% upper, and it is not deployment ranking or a forecast.
