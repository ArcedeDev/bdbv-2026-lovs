...
The current public artifact is a 15 June 2026 analytic snapshot: headline counts are dated 15 June 2026 (reviewed INSP SitRep #032 promotion) and DRC INSP per-health-zone attribution is dated 15 June 2026 from the reviewed INSP SitRep Table 2. Based on the public source package in this repo:

- **856 confirmed cases** as the current country-scope confirmed endpoint in [`data/public_snapshot.json`](data/public_snapshot.json). Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **198 confirmed deaths** as the cumulative confirmed-death endpoint.
- **743 confirmed cases officially zone-attributed across 31 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv) are the primary descriptive spread signal. The 856-case headline leaves **113 confirmed cases unallocated** to a public health-zone row: 94 DRC confirmed cases are explicitly unventilated by health zone in SitRep #032, and 19 Uganda confirmed cases have public country/district reporting but no promoted Uganda health-zone source-load.
- **208 suspected cases in isolation at 2026-06-15** in the separate `operational_status` block. SitRep #032 does not republish the under-investigation stock or total active suspected queue; those values are not fabricated. This is a point-in-time operational caseload, national-only, not a cumulative case count, and never added to confirmed.
