...
The current public artifact is a 22 August 2026 analytic snapshot. INSP released SitReps #099 and #100 after the prior package, and the snapshot's `as_of` remains the last day the data actually covers, not merely the day the documents appeared. National headlines and the per-health-zone allocation both come from SitRep #100, whose report/data clock is 22 August 2026. **SitRep #100 reports 5514 confirmed cases and 2642 confirmed deaths in DRC.** The outbreak's official geography now covers 57 of 151 health zones across six DRC provinces after SitRep #099 added Mutwanga in Nord-Kivu; SitRep #100 reports no further newly affected health zone. Based on the public source package in this repo:

- **5534 confirmed cases** as the current country-scope endpoint in [`data/public_snapshot.json`](data/public_snapshot.json): **5514 DRC + 20 Uganda**. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **2644 confirmed deaths** as the country-scope endpoint: **2642 DRC + 2 Uganda**; country-scope recovered is **1211** (**1200 DRC + 11 Uganda**).
- **5534 confirmed cases country-scope, of which 5514 confirmed cases are officially zone-attributed across 57 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv), using the reviewed INSP per-zone table. The **20 confirmed cases** of source-attribution lag are exactly the separately sourced Uganda anchor. SitRep #100's named zone rows sum exactly to the DRC headline, while its printed 233-death unventilated residual is kept out of mapped health zones.
- The descriptive **511-corridor watchlist** uses only those official source-zone rows; it carries a **0.5-100.0% lower** and **1.5-100.0% upper** adjusted-50 range as source-load context, not as an action-priority list or forecast.
- **808 people in isolation/CTEs at 2026-08-22**, carried on the current province/national response axis. SitRep #100 publishes no national confirmed-versus-suspected split, so all 808 remain unclassified by case status. The printed province care blocks sum to 800 when Bas-Uele's qualitative-only care note is left null: Ituri 528, Nord-Kivu 167, Tshopo 4, Haut-Uele 72 and Sud-Kivu 29. The resulting eight-patient residual is preserved rather than fabricated into a province or case-status split. The census is point-in-time operational state, not a cumulative case count, and is never added to confirmed.
- **14277 of 17127 contacts seen (83.4%)** on the printed national row. The provincial rows sum exactly to the national denominator in SitRep #100: Ituri 86.3%, Tshopo 95.0%, Nord-Kivu 87.4%, Haut-Uele 53.0%, Bas-Uele 80.0% and Sud-Kivu 100.0%. The denominator drop from SitRep #099 means the contact stock is not treated as a fixed cohort.
- **SitReps #099 and #100 still split their same-day deaths across community and intra-CTE components.** SitRep #099 advances by 49 confirmed deaths (27 community + 22 intra-CTE), and SitRep #100 advances by 36 (23 community + 13 intra-CTE). The combined values close on cumulative death movement and are promoted; community-only headline snippets are preserved as subsets, not as the full daily death count. SitRep #100 reports 55 new confirmations plus a one-case Haut-Uele cumulative reconciliation, so the cumulative confirmed step from SitRep #099 is 56 without inflating the daily-new series.
