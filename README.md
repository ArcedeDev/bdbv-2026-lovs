...
The current public artifact is a 10 September 2026 analytic snapshot. National headlines and the per-health-zone allocation come from visually reviewed INSP SitRep #119, whose report/data clock is 10 September 2026 and whose official PDF was published 11 September. **SitRep #119 reports 7022 confirmed cases and 3398 confirmed deaths in DRC.** Bulu opens Sud-Ubangi as the seventh affected province, taking the footprint to **62 of 167 health zones**. The denominator grew from 151 to 167 because the full 16-zone Sud-Ubangi roster entered scope; the resulting 40.4% to 37.1% change is not a continuous improvement signal. Based on the public source package in this repo:

- **7042 confirmed cases** as the current country-scope endpoint in [`data/public_snapshot.json`](data/public_snapshot.json): **7022 DRC + 20 Uganda**. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **3400 confirmed deaths** as the country-scope endpoint: **3398 DRC + 2 Uganda**; country-scope recovered is **1682** (**1671 DRC + 11 Uganda**).
- **7042 confirmed cases country-scope, of which 7022 confirmed cases are officially zone-attributed across 62 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv), using the reviewed INSP per-zone table. The **20 confirmed cases** of source-attribution lag are exactly the separately sourced Uganda anchor. SitRep #119's named zone rows sum exactly to the DRC headline, while its printed 427-death unventilated residual is kept out of mapped health zones.
...
- **837 people in isolation/CTEs at 2026-09-10** on the current province/national response axis. Five province stocks close exactly to the headline; Bas-Uele and newly affected Sud-Ubangi publish no care block. The national stock is direct, but INSP does not publish a complete national capacity denominator; it remains unset.
- **25117 of 28672 contacts seen (87.6%)** on the five printed province rows. Sud-Kivu did not report and Sud-Ubangi contact listing is still underway; missing province values remain null rather than being converted to zero.
- **SitRep #119 splits same-day deaths across community and intra-CTE components.** It advances by 49 confirmed deaths (31 community + 18 intra-CTE) and 80 confirmations. The combined death value closes on cumulative movement; the community-only cover value is a subset.
