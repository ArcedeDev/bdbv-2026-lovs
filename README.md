...
The current public artifact is a 24 July 2026 analytic snapshot: headline counts are dated 24 July 2026 (reviewed INSP SitRep #071 promotion) and DRC INSP per-health-zone attribution is current to 24 July 2026 from the reviewed SitRep #071 Tableau 2 full cumulative zone table. **Unlike the previous snapshot, this cycle declares NO harmonization: the cumulative is cleanly additive (2973 + 102 = 3075 confirmed; 1309 + 45 = 1354 deaths), the count-reconciliation gate classifies both metrics as reconciled with a zero gap, and the deltas ARE ordinary 24-hour incidence.** Based on the public source package in this repo:

- **3095 confirmed cases** as the current country-scope confirmed endpoint in [`data/public_snapshot.json`](data/public_snapshot.json), composed of **3075 confirmed cases** across the five affected DRC provinces (Ituri, Nord-Kivu, Sud-Kivu, Haut-Uele, Tshopo) plus the 20-case Uganda anchor. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **1356 confirmed deaths** as the cumulative confirmed-death endpoint.
- **3075 confirmed cases officially zone-attributed across 48 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv) are the primary descriptive spread signal. The 3095-case headline leaves **20 confirmed cases unallocated** to a public health-zone row, all of them Uganda confirmed cases with public country/district reporting but no promoted Uganda health-zone source-load. The DRC source-load remains fully zone-attributed with no unventilated residual. ONE NEW AFFECTED HEALTH ZONE was notified this cycle: Rungu (Haut-Uele) enters at 1 confirmed / 0 deaths with that case notified in the day column, taking the roster from 47 to 48 health zones touched of 140, 47 of them carrying active transmission because Miti-Murhesa (Sud-Kivu) remains in residual surveillance. Haut-Uele advances from four to five affected zones. Because the cumulative is cleanly additive this cycle, per-zone deltas ARE ordinary incidence rather than a restatement. Each zone is rendered at its own GRID3 v8.0 polygon centroid and is never collapsed into a neighbouring zone; Rungu is rendered at the interior-verified area-centroid of GRID3 zscode CD5302ZS02, 72 to 78 km from every already-registered Haut-Uele zone.
- **464 suspected cases in isolation at 2026-07-24** in the separate `operational_status` block. SitRep #071 publishes a fresh Tableau 6 patient-movement table: 766 in isolation at 89.1% occupancy over 860 beds, whose confirmed/suspect split (278 / 488) closes exactly against the census and whose bed identity closes at every level (724 J-1 + 169 admissions - 127 exits = 766). Ituri bed capacity is flat at 709 across SitReps #069-070 and its occupancy rose 77.7% to 80.4% on real admissions. Nord-Kivu is the acute capacity emergency and it WORSENED sharply, from 119.1% to a series-high 136.2% on unchanged 141 beds (168 to 192 patients), reversing the single-cycle improvement recorded at SitRep #069 and exceeding the prior high of 129.1%. National capacity moved 858 to 860 beds as Tshopo went from 8 to 10. The under-investigation stock and total active suspected queue are not republished, and those values are not fabricated. This is a point-in-time operational caseload, national-only, not a cumulative case count, and never added to confirmed.
...
For the current contract, the 20 confirmed cases not officially zone-attributed are the Uganda country anchor and remain explicit cross-border source-attribution lag rather than being redistributed onto a health-zone row. Confirmed-death attribution retains the 1-3 week trailing-note disclosure; SitRep #071 prints a full cumulative per-zone table for the coherent 48-zone DRC vector, with no DRC unventilated residual. The corridor surface is a descriptive 430-corridor watchlist over the reviewed source-load vector, unchanged this cycle because no new source zone was affected; its adjusted 50% interval ranges span 0.5-99.0% lower and 1.5-100.0% upper, with the top corridor bunia -> kisangani-cod at 99.0-100.0%, and it does not recommend deployment or forecast spread.
