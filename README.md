...
The current public artifact is a 28 August 2026 analytic snapshot. National headlines and the per-health-zone allocation come from visually reviewed INSP SitRep #106, whose report/data clock is 28 August 2026 and whose official PDF was published on 29 August. **SitRep #106 reports 5945 confirmed cases and 2862 confirmed deaths in DRC.** The outbreak's official geography covers 60 of 151 health zones across six DRC provinces after SitRep #104 added Biena and Manguredjipa in Nord-Kivu; SitRep #106 reports no further newly affected health zone. Based on the public source package in this repo:

- **5965 confirmed cases** as the current country-scope endpoint in [`data/public_snapshot.json`](data/public_snapshot.json): **5945 DRC + 20 Uganda**. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **2864 confirmed deaths** as the country-scope endpoint: **2862 DRC + 2 Uganda**; country-scope recovered is **1338** (**1327 DRC + 11 Uganda**).
- **5965 confirmed cases country-scope, of which 5945 confirmed cases are officially zone-attributed across 60 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv), using the reviewed INSP per-zone table. The **20 confirmed cases** of source-attribution lag are exactly the separately sourced Uganda anchor. SitRep #106's named zone rows sum exactly to the DRC headline, while its printed 307-death unventilated residual is kept out of mapped health zones.
- The descriptive **538-corridor watchlist** uses only those official source-zone rows; it carries a **0.4-100.0% lower** and **1.5-100.0% upper** adjusted-50 range as source-load context, not as an action-priority list or forecast.
- **795 people in isolation/CTEs at 2026-08-28**, carried on the current province/national response axis. SitRep #106 publishes no national confirmed-versus-suspected split, so all 795 remain unclassified by case status. The census is point-in-time operational state, not a cumulative case count, and is never added to confirmed.
...
- **SitReps #101 through #105 split same-day deaths across community and intra-CTE components.** SitRep #106 advances by 38 confirmed deaths (23 community + 15 intra-CTE) and 69 confirmations. The combined death value closes on cumulative movement and is promoted; community-only headline snippets remain subsets, not the full daily death count.
