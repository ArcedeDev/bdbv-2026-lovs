...
The current public artifact is a 6 September 2026 analytic snapshot. National headlines and the per-health-zone allocation come from visually reviewed INSP SitRep #115, whose report/data clock is 6 September 2026 and whose official PDF was posted on 7 September. **SitRep #115 reports 6686 confirmed cases and 3226 confirmed deaths in DRC.** The outbreak's official geography covers 61 of 151 health zones across six DRC provinces, unchanged this cut. Based on the public source package in this repo:

- **6706 confirmed cases** as the current country-scope endpoint in [`data/public_snapshot.json`](data/public_snapshot.json): **6686 DRC + 20 Uganda**. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **3228 confirmed deaths** as the country-scope endpoint: **3226 DRC + 2 Uganda**; country-scope recovered is **1574** (**1563 DRC + 11 Uganda**).
- **6706 confirmed cases country-scope, of which 6686 confirmed cases are officially zone-attributed across 61 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv), using the reviewed INSP per-zone table. The **20 confirmed cases** of source-attribution lag are exactly the separately sourced Uganda anchor. SitRep #115's named zone rows sum exactly to the DRC headline, while its printed 378-death unventilated residual is kept out of mapped health zones.
- The descriptive **547-corridor watchlist** uses only those official source-zone rows; it carries a **0.5-100.0% lower** and **1.4-100.0% upper** adjusted-50 range as source-load context, not as an action-priority list or forecast.
- **819 people in isolation/CTEs at 2026-09-06**, carried on the current province/national response axis. The national stock is direct, but INSP does not publish a complete national capacity denominator; it remains unset. SitRep #115 publishes no national confirmed-versus-suspected split, so the status categories remain unclassified. The census is point-in-time operational state, not a cumulative case count, and is never added to confirmed.
