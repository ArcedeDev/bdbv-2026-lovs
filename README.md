...
The current public artifact is a 18 July 2026 analytic snapshot: headline counts are dated 18 July 2026 (reviewed INSP SitRep #065 promotion) and DRC INSP per-health-zone attribution is current to 18 July 2026 from the reviewed SitRep #065 Tableau 2 full cumulative zone table. Based on the public source package in this repo:

- **2364 confirmed cases** as the current country-scope confirmed endpoint in [`data/public_snapshot.json`](data/public_snapshot.json), composed of **2344 confirmed cases** across the five affected DRC provinces (Ituri, Nord-Kivu, Sud-Kivu, Haut-Uele, Tshopo) plus the 20-case Uganda anchor. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **932 confirmed deaths** as the cumulative confirmed-death endpoint.
- **2327 confirmed cases officially zone-attributed across 46 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv) are the primary descriptive spread signal. The 2364-case headline leaves **37 confirmed cases unallocated** to a public health-zone row: 17 DRC Ituri confirmed cases are explicitly unventilated (awaiting health-zone identification), and 20 Uganda confirmed cases have public country/district reporting but no promoted Uganda health-zone source-load. No new affected health zone was notified this cycle; the roster holds at 46 health zones touched, 45 of them carrying active transmission because Miti-Murhesa (Sud-Kivu) prints 52 days with no new confirmed case and remains in residual surveillance.
- **452 suspected cases in isolation at 2026-07-18** in the separate `operational_status` block. SitRep #065 publishes a fresh Tableau 6 patient-movement table: 724 in isolation at 84.5% occupancy over 857 beds, whose confirmed/suspect split (272 / 452) closes exactly against the census and whose bed identity closes at every level (707 J-1 + 121 admissions - 104 exits = 724). The under-investigation stock and total active suspected queue are not republished, and those values are not fabricated. This is a point-in-time operational caseload, national-only, not a cumulative case count, and never added to confirmed.
