...
- **451 suspected cases in isolation at 2026-07-20** in the separate `operational_status` block. SitRep #067 publishes a fresh Tableau 6 patient-movement table: 737 in isolation at 86.7% occupancy over 850 beds, whose confirmed/suspect split (286 / 451) closes exactly against the census and whose bed identity closes at every level (700 J-1 + 180 admissions - 143 exits = 737). Ituri bed capacity contracted 798 -> 709 while its census moved only 553 -> 556, so the province occupancy rise to 78.4% is a denominator revision rather than an admissions surge. The under-investigation stock and total active suspected queue are not republished, and those values are not fabricated. This is a point-in-time operational caseload, national-only, not a cumulative case count, and never added to confirmed.
