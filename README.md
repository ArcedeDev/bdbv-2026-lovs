...
The current public artifact is a 25 July 2026 analytic snapshot: headline counts are dated 25 July 2026 (reviewed INSP SitRep #072 promotion) and DRC INSP per-health-zone attribution is current to 25 July 2026 from the reviewed SitRep #072 Tableau 2 full cumulative zone table. **This cycle DOES declare a harmonization, but it is additive rather than a restatement: one case in Wamba (Haut-Uele) after provincial database harmonization, already inside the notified +125. The cumulative still closes exactly (3075 + 125 = 3200 confirmed; 1354 + 51 = 1405 deaths), the count-reconciliation gate classifies both metrics as reconciled with a zero gap, and the deltas ARE ordinary 24-hour incidence.** Based on the public source package in this repo:

- **3220 confirmed cases** as the current country-scope confirmed endpoint in [`data/public_snapshot.json`](data/public_snapshot.json), composed of **3200 confirmed cases** across the five affected DRC provinces (Ituri, Nord-Kivu, Sud-Kivu, Haut-Uele, Tshopo) plus the 20-case Uganda anchor. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **1407 confirmed deaths** as the cumulative confirmed-death endpoint.
- **3200 confirmed cases officially zone-attributed across 48 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv) are the primary descriptive spread signal. The 3220-case headline leaves **20 confirmed cases unallocated** to a public health-zone row, all of them Uganda confirmed cases with public country/district reporting but no promoted Uganda health-zone source-load. The DRC source-load remains fully zone-attributed with no unventilated residual. NO NEW AFFECTED HEALTH ZONE was notified this cycle: the roster holds at 48 health zones touched of 140, 47 of them carrying active transmission because Miti-Murhesa (Sud-Kivu) remains in residual surveillance at 60 days without a confirmed case. Because the cumulative is additive this cycle, per-zone deltas ARE ordinary incidence rather than a restatement; Rwampara is the largest single-zone movement at +35 confirmed with only 2 deaths in its day column, both intra-CTE. Each zone is rendered at its own GRID3 v8.0 polygon centroid and is never collapsed into a neighbouring zone.
- **466 suspected cases in isolation at 2026-07-25** in the separate `operational_status` block. SitRep #072 publishes a fresh Tableau 6 patient-movement table: 773 in isolation at 82.1% occupancy over 941 beds, whose confirmed/suspect split (307 / 466) closes exactly against the census and whose bed identity closes at every level (743 J-1 + 151 admissions - 121 exits = 773). Ituri bed capacity JUMPED from 709 to 800, the largest single-cycle addition of the recent series and consistent with the Rwampara CTE reaching 100 operational beds; that addition, not a fall in patients, is what moves Ituri occupancy from 81.2% to 75.3% on a census that rose from 576 to 602. Nord-Kivu remains the acute capacity emergency at 121.3% on a bed count frozen at 141 for a third consecutive cycle, and it is the only province over capacity. National capacity moved from 850 to 941 beds entirely on the Ituri addition. Haut-Uele and Tshopo again report no bed data. The under-investigation stock and total active suspected queue are not republished, and those values are not fabricated. This is a point-in-time operational caseload, national-only, not a cumulative case count, and never added to confirmed.
...
For the current contract, the 20 confirmed cases not officially zone-attributed are the Uganda country anchor and remain explicit cross-border source-attribution lag rather than being redistributed onto a health-zone row. Confirmed-death attribution retains the 1-3 week trailing-note disclosure; SitRep #072 prints a full cumulative per-zone table for the coherent 48-zone DRC vector, with no DRC unventilated residual. The corridor surface is a descriptive 430-corridor watchlist over the reviewed source-load vector, unchanged this cycle because no new source zone was affected; its adjusted 50% interval ranges span 0.5-99.0% lower and 1.5-100.0% upper, with the top corridor bunia -> kasese-uga at 99.0-100.0%, and it does not recommend deployment or forecast spread.
