...
The current public artifact is a 4 August 2026 analytic snapshot: headline counts and the DRC INSP per-health-zone attribution come from the visually reviewed SitRep #082 tables. **All 51 printed rows close exactly to 3973 confirmed cases and 1801 confirmed deaths in DRC, and every province total matches Table 1: the two-death North Kivu residual carried in SitRep #081 is resorbed here. The footprint remains 51 zones. The notified 24-hour movement is +99 confirmed cases and +52 confirmed deaths; the national death cumulative advances only 50 because it also absorbs that two-unit correction, so the notified count, which matches the per-zone table exactly, is the day's true movement.** Based on the public source package in this repo:

- **3993 confirmed cases** as the current country-scope endpoint in [`data/public_snapshot.json`](data/public_snapshot.json): **3973 DRC + 20 Uganda**. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **1803 confirmed deaths** as the country-scope endpoint: **1801 DRC + 2 Uganda**.
- **3973 confirmed cases officially zone-attributed across 51 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv). The 20-case source-attribution lag against the country-scope headline is exactly the separately sourced Uganda country anchor; no DRC confirmed case is unallocated, and death attribution now closes exactly as well.
- **343 suspected cases in isolation at 2026-08-04** in the separate `operational_status` block. The source also classifies 311 isolated patients as confirmed but does not split Tshopo's 3 or Sud-Kivu's 17 by status, so the package preserves those 20 as unclassified; the three components close to the national census of 674 without inference. Haut-Uele reports no isolation census at all this cycle, so it is absent from the 674 rather than counted as zero. Only Ituri publishes a bed denominator, **468/833 (56.2%)**. A national occupancy rate is not calculated because the source publishes no patient-movement table this cycle. This is a point-in-time operational caseload, not a cumulative case count, and is never added to confirmed.
