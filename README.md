...
The current public artifact is a 2 September 2026 analytic snapshot. National headlines and the per-health-zone allocation come from visually reviewed INSP SitRep #111, whose report/data clock is 2 September 2026 and whose official PDF was posted on 4 September (the edition itself states a 3 September publication date). **SitRep #111 reports 6342 confirmed cases and 3072 confirmed deaths in DRC.** The outbreak's official geography covers 60 of 151 health zones across six DRC provinces; SitRep #111 reports no further newly affected health zone. Based on the public source package in this repo:

- **6362 confirmed cases** as the current country-scope endpoint in [`data/public_snapshot.json`](data/public_snapshot.json): **6342 DRC + 20 Uganda**. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **3074 confirmed deaths** as the country-scope endpoint: **3072 DRC + 2 Uganda**; country-scope recovered is **1486** (**1475 DRC + 11 Uganda**).
- **6362 confirmed cases country-scope, of which 6342 confirmed cases are officially zone-attributed across 60 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv), using the reviewed INSP per-zone table. The **20 confirmed cases** of source-attribution lag are exactly the separately sourced Uganda anchor. SitRep #111's named zone rows sum exactly to the DRC headline, while its printed 351-death unventilated residual is kept out of mapped health zones.
- The descriptive **538-corridor watchlist** uses only those official source-zone rows; it carries a **0.5-100.0% lower** and **1.5-100.0% upper** adjusted-50 range as source-load context, not as an action-priority list or forecast.
- **770 people in isolation/CTEs at 2026-09-02**, carried on the current province/national response axis. Only four of six provinces publish an isolation block this edition, and those four sum to 758, twelve short of the printed national total; the gap is preserved rather than distributed. SitRep #111 publishes no national confirmed-versus-suspected split, so the status categories remain unclassified. The census is point-in-time operational state, not a cumulative case count, and is never added to confirmed.
- **18520 of 22463 contacts seen (82.4%)** on the printed national row. Sud-Kivu did not report; missing province values remain null rather than being converted to zero. The changing denominator means the contact stock is not treated as a fixed cohort.
- **SitRep #111 splits same-day deaths across community and intra-CTE components.** It advances by 33 confirmed deaths (24 community + 9 intra-CTE) and 92 confirmations. The combined death value closes on cumulative movement and is promoted; community-only headline snippets remain subsets, not the full daily death count.
