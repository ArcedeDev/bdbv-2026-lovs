...
The current public artifact is a 15 July 2026 analytic snapshot: headline counts are dated 15 July 2026 (reviewed INSP SitRep #062 promotion) and DRC INSP per-health-zone attribution is current to 15 July 2026 from the reviewed INSP SitRep Table 2 zone list. Based on the public source package in this repo:

- **2144 confirmed cases** as the current country-scope confirmed endpoint in [`data/public_snapshot.json`](data/public_snapshot.json), composed of **2124 confirmed cases** across the five affected DRC provinces (Ituri, Nord-Kivu, Sud-Kivu, Haut-Uele, Tshopo) plus the 20-case Uganda anchor. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **830 confirmed deaths** as the cumulative confirmed-death endpoint.
- **2107 confirmed cases officially zone-attributed across 46 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv) are the primary descriptive spread signal. The 2144-case headline leaves **37 confirmed cases unallocated** to a public health-zone row: 17 DRC Ituri confirmed cases are explicitly unventilated (awaiting health-zone identification), and 20 Uganda confirmed cases have public country/district reporting but no promoted Uganda health-zone source-load. Mahagi was notified as a new affected health zone this cycle (its first confirmed case, on the Goli-Mahagi/Arua cross-border axis), taking the outbreak to 46 health zones touched; 45 of those carry active transmission, because Miti-Murhesa (Sud-Kivu) reached 50 days with no new confirmed case and moved into residual surveillance.
- **444 suspected cases in isolation at 2026-07-15** in the separate `operational_status` block. SitRep #062 publishes a fresh Tableau 6 patient-movement table: 725 in isolation at 85.6% occupancy, split 281 confirmed / 444 suspected; the under-investigation stock and total active suspected queue are not republished, and those values are not fabricated. This is a point-in-time operational caseload, national-only, not a cumulative case count, and never added to confirmed.
...
For the current contract, the 37 confirmed cases not yet officially zone-attributed are treated as source-attribution lag, not as hidden model redistribution. Confirmed-death attribution retains the 1-3 week trailing-note disclosure; SitRep #062 republishes per-zone confirmed deaths for the coherent 46-zone vector, with the 17 Ituri unventilated cases held in the explicit unallocated residual. The corridor surface is a descriptive 412-corridor watchlist over the reviewed source-load vector; its adjusted 50% interval ranges span 0.5-96.8% lower and 1.4-100.0% upper, and it is not deployment ranking or a forecast.
