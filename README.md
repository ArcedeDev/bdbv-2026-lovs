...
The current public artifact is a 20 July 2026 analytic snapshot: headline counts are dated 20 July 2026 (reviewed INSP SitRep #067 promotion) and DRC INSP per-health-zone attribution is current to 20 July 2026 from the reviewed SitRep #067 Tableau 2 full cumulative zone table. Based on the public source package in this repo:

- **2493 confirmed cases** as the current country-scope confirmed endpoint in [`data/public_snapshot.json`](data/public_snapshot.json), composed of **2473 confirmed cases** across the five affected DRC provinces (Ituri, Nord-Kivu, Sud-Kivu, Haut-Uele, Tshopo) plus the 20-case Uganda anchor. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **1001 confirmed deaths** as the cumulative confirmed-death endpoint.
- **2456 confirmed cases officially zone-attributed across 47 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv) are the primary descriptive spread signal. The 2493-case headline leaves **37 confirmed cases unallocated** to a public health-zone row: 17 DRC Ituri confirmed cases are explicitly unventilated (awaiting health-zone identification), and 20 Uganda confirmed cases have public country/district reporting but no promoted Uganda health-zone source-load. NO new affected health zone was notified this cycle: the roster holds at 47 health zones touched, 46 of them carrying active transmission because Miti-Murhesa (Sud-Kivu) remains in residual surveillance at 54 days without a confirmed case. Every cumulative zone delta against the previous cycle equals that zone's own printed day column, so no row was silently restated. Each zone is rendered at its own GRID3 v8.0 polygon centroid and is never collapsed into a neighbouring zone.
- **451 suspected cases in isolation at 2026-07-20** in the separate `operational_status` block. SitRep #067 publishes a fresh Tableau 6 patient-movement table: 737 in isolation at 86.7% occupancy over 850 beds, whose confirmed/suspect split (286 / 451) closes exactly against the census and whose bed identity closes at every level (700 J-1 + 180 admissions - 143 exits = 737). Ituri bed capacity did not contract: the printed series runs 693, 693, 700, 706, 798, 709 across SitReps 62-67, so SitRep #066's 798 is a one-cycle outlier and this cycle's 709 continues steady growth. The apparent occupancy move from 69.3% to 78.4% corrects the prior cycle's denominator; Ituri has held a ~78% plateau all week. The under-investigation stock and total active suspected queue are not republished, and those values are not fabricated. This is a point-in-time operational caseload, national-only, not a cumulative case count, and never added to confirmed.
