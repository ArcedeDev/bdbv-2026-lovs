...
The current public artifact is a 9 July 2026 analytic snapshot: headline counts are dated 9 July 2026 (reviewed INSP SitRep #056 promotion) and DRC INSP per-health-zone attribution is dated 9 July 2026 from the reviewed INSP SitRep section 3.2 zone list. Based on the public source package in this repo:

- **1850 confirmed cases** as the current country-scope confirmed endpoint in [`data/public_snapshot.json`](data/public_snapshot.json), composed of **1830 confirmed cases** across the three affected DRC provinces (Ituri, Nord-Kivu, Sud-Kivu) plus the 20-case Uganda anchor. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **650 confirmed deaths** as the cumulative confirmed-death endpoint.
- **1813 confirmed cases officially zone-attributed across 37 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv) are the primary descriptive spread signal. The 1850-case headline leaves **37 confirmed cases unallocated** to a public health-zone row: 17 DRC Ituri confirmed cases are explicitly awaiting health-zone identification in the coherent SitRep #056 Table 2 totals (Nord-Kivu Table 1 equals its Table 2 named sum this cycle, so there is no Nord-Kivu ventilation gap), and 20 Uganda confirmed cases have public country/district reporting but no promoted Uganda health-zone source-load.
- **542 suspected cases in isolation at 2026-07-09** in the separate `operational_status` block. SitRep #056 publishes a fresh Tableau 7 patient-movement table: 780 in isolation at 100.3% occupancy, split 238 confirmed / 542 suspected; the under-investigation stock and total active suspected queue are not republished, and those values are not fabricated. This is a point-in-time operational caseload, national-only, not a cumulative case count, and never added to confirmed.
...
For the current contract, the 37 confirmed cases not yet officially zone-attributed are treated as source-attribution lag, not as hidden model redistribution. Confirmed-death attribution retains the 1-3 week trailing-note disclosure; SitRep #056 republishes per-zone confirmed deaths for the coherent 37-zone vector, with no newly named zone this cycle and 17 Ituri confirmed cases held in the explicit unventilated residual (Nord-Kivu Table 1 equals its Table 2 named sum this cycle, so there is no Nord-Kivu ventilation gap). The corridor surface is a descriptive 331-corridor watchlist over the reviewed source-load vector; its adjusted 50% interval ranges span 0.5-95.8% lower and 1.5-100.0% upper, and it is not deployment ranking or a forecast.
