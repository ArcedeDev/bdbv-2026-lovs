...
The current public artifact is a 11 July 2026 analytic snapshot: headline counts are dated 11 July 2026 (reviewed INSP SitRep #058 promotion) and DRC INSP per-health-zone attribution is dated 11 July 2026 from the reviewed INSP SitRep section 3.2 zone list. Based on the public source package in this repo:

- **1946 confirmed cases** as the current country-scope confirmed endpoint in [`data/public_snapshot.json`](data/public_snapshot.json), composed of **1926 confirmed cases** across the five affected DRC provinces (Ituri, Nord-Kivu, Sud-Kivu, Haut-Uele, Tshopo) plus the 20-case Uganda anchor. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **704 confirmed deaths** as the cumulative confirmed-death endpoint.
- **1903 confirmed cases officially zone-attributed across 37 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv) are the primary descriptive spread signal. The 1946-case headline leaves **43 confirmed cases unallocated** to a public health-zone row: 17 DRC Ituri confirmed cases plus 6 confirmed cases in newly-listed un-geolocated zones are explicitly awaiting health-zone identification in the coherent SitRep #058 Table 2 totals, and 20 Uganda confirmed cases have public country/district reporting but no promoted Uganda health-zone source-load.
- **505 suspected cases in isolation at 2026-07-11** in the separate `operational_status` block. SitRep #058 publishes a fresh Tableau 7 patient-movement table: 753 in isolation at 93.9% occupancy, split 248 confirmed / 505 suspected; the under-investigation stock and total active suspected queue are not republished, and those values are not fabricated. This is a point-in-time operational caseload, national-only, not a cumulative case count, and never added to confirmed.
...
For the current contract, the 43 confirmed cases not yet officially zone-attributed are treated as source-attribution lag, not as hidden model redistribution. Confirmed-death attribution retains the 1-3 week trailing-note disclosure; SitRep #058 republishes per-zone confirmed deaths for the coherent 37-zone vector, with 17 Ituri confirmed cases plus 6 confirmed cases in newly-listed un-geolocated zones held in the explicit unallocated residual. The corridor surface is a descriptive 331-corridor watchlist over the reviewed source-load vector; its adjusted 50% interval ranges span 0.5-95.5% lower and 1.5-100.0% upper, and it is not deployment ranking or a forecast.
