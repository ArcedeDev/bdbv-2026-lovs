...
The current public artifact is a 17 June 2026 analytic snapshot: headline counts are dated 17 June 2026 (reviewed INSP SitRep #034 promotion) and DRC INSP per-health-zone attribution is dated 17 June 2026 from the reviewed INSP SitRep section 3.2 zone list. Based on the public source package in this repo:

- **915 confirmed cases** as the current country-scope confirmed endpoint in [`data/public_snapshot.json`](data/public_snapshot.json). Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **234 confirmed deaths** as the cumulative confirmed-death endpoint.
- **879 confirmed cases officially zone-attributed across 33 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv) are the primary descriptive spread signal. The 915-case headline leaves **36 confirmed cases unallocated** to a public health-zone row: 17 DRC confirmed cases are explicitly unventilated by health zone in the coherent SitRep #034 Table 2 totals, and 19 Uganda confirmed cases have public country/district reporting but no promoted Uganda health-zone source-load.
- **222 suspected cases in isolation at 2026-06-17** in the separate `operational_status` block. SitRep #034 does not republish the under-investigation stock or total active suspected queue; those values are not fabricated. This is a point-in-time operational caseload, national-only, not a cumulative case count, and never added to confirmed.
...
For the current contract, the 36 confirmed cases not yet officially zone-attributed are treated as source-attribution lag, not as hidden model redistribution. Confirmed-death attribution retains the 1-3 week trailing-note disclosure; SitRep #034 republishes per-zone confirmed deaths for the coherent 33-zone vector, while its printed Drodro row is held out as a source-internal table conflict because including it would break the published Ituri and national totals. The corridor surface is a descriptive 295-corridor watchlist over the reviewed source-load vector; its adjusted 50% interval ranges span 0.5-80.0% lower and 1.4-98.7% upper, and it is not deployment ranking or a forecast.
