...
The current public artifact is a 22 July 2026 analytic snapshot: headline counts are dated 22 July 2026 (reviewed INSP SitRep #069 promotion) and DRC INSP per-health-zone attribution is current to 22 July 2026 from the reviewed SitRep #069 Tableau 2 full cumulative zone table. **This cycle carries a source-declared harmonization of the Ituri and Nord-Kivu provincial databases with the national level, so the cumulative is NOT additive from the previous snapshot and its deltas are not 24-hour incidence.** Based on the public source package in this repo:

- **2925 confirmed cases** as the current country-scope confirmed endpoint in [`data/public_snapshot.json`](data/public_snapshot.json), composed of **2905 confirmed cases** across the five affected DRC provinces (Ituri, Nord-Kivu, Sud-Kivu, Haut-Uele, Tshopo) plus the 20-case Uganda anchor. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **1271 confirmed deaths** as the cumulative confirmed-death endpoint.
- **2905 confirmed cases officially zone-attributed across 47 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv) are the primary descriptive spread signal. The 2925-case headline leaves **20 confirmed cases unallocated** to a public health-zone row, all of them Uganda confirmed cases with public country/district reporting but no promoted Uganda health-zone source-load. The 17 DRC Ituri unventilated cases carried since SitRep #053 are gone: the harmonization allocated the residual, and Ituri's 28 named zone rows now sum exactly to its provincial total of 2595, so the DRC source-load is fully zone-attributed for the first time. NO new affected health zone was notified this cycle: the roster holds at 47 health zones touched of 140, 46 of them carrying active transmission because Miti-Murhesa (Sud-Kivu) remains in residual surveillance at 55 days without a confirmed case. Cumulative zone deltas this cycle are a harmonized restatement rather than incidence, and the restatement is bidirectional: Butembo, Ariwara, Aungba, Boga, Drodro and Mambasa all revise down while Mongbwalu, Nizi, Mangala, Lita and Nia-Nia revise up. Each zone is rendered at its own GRID3 v8.0 polygon centroid and is never collapsed into a neighbouring zone.
- **437 suspected cases in isolation at 2026-07-22** in the separate `operational_status` block. SitRep #069 publishes a fresh Tableau 6 patient-movement table: 722 in isolation at 84.1% occupancy over 858 beds, whose confirmed/suspect split (285 / 437) closes exactly against the census and whose bed identity closes at every level (742 J-1 + 126 admissions - 146 exits = 722). Ituri bed capacity is flat: the printed series runs 693, 693, 700, 706, 798, 709, 708, 709 across SitReps 62-69, holding its ~78% occupancy plateau (78.5% to 77.7%) and keeping SitRep #066's 798 bracketed as a one-cycle data-entry outlier. Nord-Kivu remains the one genuine saturation but eased from 129.1% to 119.1% on unchanged 141 beds, the first improvement in that series. National capacity moved 849 to 858 beds because Tshopo reported bed capacity (8 beds) for the first time, so the occupancy move is partly a composition change and not purely decompression. The under-investigation stock and total active suspected queue are not republished, and those values are not fabricated. This is a point-in-time operational caseload, national-only, not a cumulative case count, and never added to confirmed.
...
For the current contract, the 37 confirmed cases not yet officially zone-attributed are treated as source-attribution lag, not as hidden model redistribution. Confirmed-death attribution retains the 1-3 week trailing-note disclosure; SitRep #068 prints a full cumulative per-zone table for the coherent 47-zone vector, with the 17 Ituri unventilated cases held in the explicit unallocated residual. The corridor surface is a descriptive 421-corridor watchlist over the reviewed source-load vector (unchanged this cycle because no source zone was added or removed); its adjusted 50% interval ranges span 0.5-98.8% lower and 1.5-100.0% upper, and it is not deployment ranking or a forecast.
