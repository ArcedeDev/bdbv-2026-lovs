...
The current public artifact is a 5 September 2026 analytic snapshot. National headlines and the per-health-zone allocation come from visually reviewed INSP SitRep #114, whose report/data clock is 5 September 2026 and whose official PDF was posted on 6 September. **SitRep #114 reports 6604 confirmed cases and 3175 confirmed deaths in DRC.** The outbreak's official geography covers 61 of 151 health zones across six DRC provinces, unchanged this cut. Based on the public source package in this repo:

- **6624 confirmed cases** as the current country-scope endpoint in [`data/public_snapshot.json`](data/public_snapshot.json): **6604 DRC + 20 Uganda**. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **3177 confirmed deaths** as the country-scope endpoint: **3175 DRC + 2 Uganda**; country-scope recovered is **1559** (**1548 DRC + 11 Uganda**).
- **6624 confirmed cases country-scope, of which 6604 confirmed cases are officially zone-attributed across 61 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv), using the reviewed INSP per-zone table. The **20 confirmed cases** of source-attribution lag are exactly the separately sourced Uganda anchor. SitRep #114's named zone rows sum exactly to the DRC headline, while its printed 366-death unventilated residual is kept out of mapped health zones.
...
- **851 people in isolation/CTEs at 2026-09-05**, carried on the current province/national response axis. The national stock is direct, but INSP does not publish a complete national capacity denominator; it remains unset. SitRep #114 publishes no national confirmed-versus-suspected split, so the status categories remain unclassified. The census is point-in-time operational state, not a cumulative case count, and is never added to confirmed.
