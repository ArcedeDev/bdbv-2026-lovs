...
The current public artifact is an 18 August 2026 analytic snapshot, published on 20 August 2026. INSP went silent for four days after SitRep #093 and then released SitReps #094, #095 and #096 together, so the publication clock and the data clock separate by two days here: the snapshot's `as_of` is the last day the data actually covers, not the day the documents appeared. National headlines and the per-health-zone allocation both come from SitRep #096, whose report/data clock is 18 August 2026. **SitRep #096 reports 5208 confirmed cases and 2476 confirmed deaths in DRC.** The outbreak's official geography widens again: SitRep #096 registers **Viadana** (Bas-Uele) as newly affected, taking the footprint to 56 of 151 health zones and Bas-Uele from one affected zone to two of its eleven. Viadana does not adjoin Buta, the province's first affected zone, and lies roughly 270 km from it; its neighbours are Isiro, Niangara and Rungu in Haut-Uele, two of which were already affected, so this is a step across the Haut-Uele frontier rather than growth of a Buta cluster. Based on the public source package in this repo:

- **5228 confirmed cases** as the current country-scope endpoint in [`data/public_snapshot.json`](data/public_snapshot.json): **5208 DRC + 20 Uganda**. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **2478 confirmed deaths** as the country-scope endpoint: **2476 DRC + 2 Uganda**; country-scope recovered is **1126** (**1115 DRC + 11 Uganda**).
- **5228 confirmed cases country-scope, of which 5208 confirmed cases are officially zone-attributed across 56 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv), using the reviewed INSP per-zone table. The **20 confirmed cases** of source-attribution lag are exactly the separately sourced Uganda anchor. SitRep #096's named zone rows sum exactly to the DRC headline, while its printed 174-death unventilated residual is kept out of mapped health zones.
- **730 people in isolation/CTEs at 2026-08-18**, carried on the current province/national response axis. SitRep #096 publishes no national confirmed-versus-suspected split, so all 730 remain unclassified by case status. Unlike every other edition in this run it does close on its published parts: Ituri 447, Nord-Kivu 205, Haut-Uele 53, Tshopo 5 and Sud-Kivu 20 sum exactly to 730. Nord-Kivu is effectively full at **205/206 beds (99.5%)**, and Ituri reports **447/965 beds occupied (46.3%)**; provincial denominators are never treated as national capacity. The census is point-in-time operational state, not a cumulative case count, and is never added to confirmed.
- **18517 of 22043 contacts seen (84.0%)**, below the 85% threshold and down from 85.3% at SitRep #093. Per province: Ituri 87.7%, Nord-Kivu 82.4%, Tshopo 75.5%, Haut-Uele 59.1%, and Bas-Uele 18.7% - the weakest in the country, in the province that just gained a zone. The printed national figures exclude Bas-Uele: the four counted provinces sum exactly to the 18517 numerator, and to 22041 against the printed 22043.
- **All three editions understate their own 24-hour death toll on the front page.** Each highlights box names only the community component (SitRep #094: 30, #095: 29, #096: 35) while the same edition's zone table prints community plus intra-CTE deaths for the same 24 hours (53, 42 and 56 respectively). The zone tables close on every province, so those totals are promoted and the front-page figures preserved as the community subsets they describe. SitRep #095 additionally prints a national contact-follow-up rate (84.1%) and an Ituri rate (93.0%) that do not follow from the counts printed beside them (83.6% and 86.9%), and a transferred-suspect count in prose (105) that its own table contradicts (118).
