...
The current public artifact is a 27 August 2026 analytic snapshot. National headlines and the per-health-zone allocation come from visually reviewed INSP SitRep #105, whose report/data clock is 27 August 2026 and whose official PDF was published on 29 August. **SitRep #105 reports 5863 confirmed cases and 2824 confirmed deaths in DRC.** The outbreak's official geography covers 60 of 151 health zones across six DRC provinces after SitRep #104 added Biena and Manguredjipa in Nord-Kivu; SitRep #105 reports no further newly affected health zone. Based on the public source package in this repo:

- **5883 confirmed cases** as the current country-scope endpoint in [`data/public_snapshot.json`](data/public_snapshot.json): **5863 DRC + 20 Uganda**. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **2826 confirmed deaths** as the country-scope endpoint: **2824 DRC + 2 Uganda**; country-scope recovered is **1329** (**1318 DRC + 11 Uganda**).
- **5883 confirmed cases country-scope, of which 5863 confirmed cases are officially zone-attributed across 60 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv), using the reviewed INSP per-zone table. The **20 confirmed cases** of source-attribution lag are exactly the separately sourced Uganda anchor. SitRep #105's named zone rows sum exactly to the DRC headline, while its printed 297-death unventilated residual is kept out of mapped health zones.
...
- **795 people in isolation/CTEs at 2026-08-27**, carried on the current province/national response axis. SitRep #105 publishes no national confirmed-versus-suspected split, so all 795 remain unclassified by case status. The census is point-in-time operational state, not a cumulative case count, and is never added to confirmed.
...
- **SitReps #101 through #105 split same-day deaths across community and intra-CTE components.** SitRep #105 advances by 38 confirmed deaths (23 community + 15 intra-CTE) and 69 confirmations. The combined death value closes on cumulative movement and is promoted; community-only headline snippets remain subsets, not the full daily death count.
