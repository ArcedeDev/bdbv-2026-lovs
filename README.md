...
The current public artifact is a 30 August 2026 analytic snapshot. National headlines and the per-health-zone allocation come from visually reviewed INSP SitRep #108, whose report/data clock is 30 August 2026 and whose official PDF was published on 31 August. **SitRep #108 reports 6100 confirmed cases and 2950 confirmed deaths in DRC.** The outbreak's official geography covers 60 of 151 health zones across six DRC provinces; SitRep #108 reports no further newly affected health zone. Based on the public source package in this repo:

- **6120 confirmed cases** as the current country-scope endpoint in [`data/public_snapshot.json`](data/public_snapshot.json): **6100 DRC + 20 Uganda**. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **2952 confirmed deaths** as the country-scope endpoint: **2950 DRC + 2 Uganda**; country-scope recovered is **1394** (**1383 DRC + 11 Uganda**).
- **6120 confirmed cases country-scope, of which 6100 confirmed cases are officially zone-attributed across 60 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv), using the reviewed INSP per-zone table. The **20 confirmed cases** of source-attribution lag are exactly the separately sourced Uganda anchor. SitRep #108's named zone rows sum exactly to the DRC headline, while its printed 329-death unventilated residual is kept out of mapped health zones.
...
- **SitRep #108 splits same-day deaths across community and intra-CTE components.** It advances by 39 confirmed deaths (19 community + 20 intra-CTE) and 59 confirmations. The combined death value closes on cumulative movement and is promoted; community-only headline snippets remain subsets, not the full daily death count.
