...
The current public artifact is a 19 August 2026 publication-day analytic snapshot. National headlines and the per-health-zone allocation both come from SitRep #093, whose report/data clock is 15 August 2026. **No counts changed in this snapshot.** INSP has published no situation report since SitRep #093 appeared on 16 August, so every count below is carried forward from the 15 August report clock and is labelled `awaiting_next_publication` in [`data/public_snapshot.json`](data/public_snapshot.json). What advanced is the source record, not the epidemiology: the INRB/INSP/UMIE data release of 19 August independently republishes the same SitRep #093 cut. **SitRep #093 reports 4945 confirmed cases and 2325 confirmed deaths in DRC.** The outbreak's official geography widened twice earlier in this run: SitRep #090 opened **Bas-Uele** as a sixth affected province with a single fatal confirmed case in the Buta health zone, and SitRep #091 added a health zone named **Tshopo** inside Tshopo province. SitRep #093 adds no new zone, so the denominator holds at 151 health zones, of which 55 are affected. Based on the public source package in this repo:

- **4965 confirmed cases** as the current country-scope endpoint in [`data/public_snapshot.json`](data/public_snapshot.json): **4945 DRC + 20 Uganda**. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **2327 confirmed deaths** as the country-scope endpoint: **2325 DRC + 2 Uganda**; country-scope recovered is **1051** (**1040 DRC + 11 Uganda**).
- **4965 confirmed cases country-scope, of which 4945 confirmed cases are officially zone-attributed across 55 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv), using the reviewed INSP per-zone table. The **20 confirmed cases** of source-attribution lag are exactly the separately sourced Uganda anchor. SitRep #093's named zone rows sum exactly to the DRC headline, while its printed 130-death unventilated residual is kept out of mapped health zones.
- **730 people in isolation/CTEs at 2026-08-15**, carried on the current province/national response axis. SitRep #093 publishes no national confirmed-versus-suspected split, so all 730 remain unclassified by case status. It is also not the sum of its published parts: the four provinces that publish a care block total 733, and Haut-Uele publishes none at all. Ituri reports **548/991 beds occupied (55.3%)**; that provincial denominator is not treated as national capacity, and the aggregate conceals saturated units at Bambu, Fataki and Nizi. The census is point-in-time operational state, not a cumulative case count, and is never added to confirmed.
- **20791 of 24383 contacts seen (85.3%)**, back above the 85% threshold and up from 82.6% at SitRep #091, on a larger denominator. This is the first edition in this run to publish a per-province contact table: Ituri 89.8%, Nord-Kivu 79.0%, Tshopo 100%, and Haut-Uele 61.8% on explicitly incomplete reporting (4 health zones did not report).
- **SitRep #093 corrects SitRep #092.** Its province summary carries a footnote that a Makiso (Tshopo) case recorded on 14 August was omitted from SitRep #092's cumulative. Tshopo therefore advances 11 to 13 on a single printed new case. The correction is INSP's own and is promoted as printed; the published SitRep #092 figures are left as INSP published them rather than retroactively edited.
- **Three days without a situation report.** SitRep #093 (published 16 August) remains the newest INSP edition as of this snapshot. The preceding editions ran close to daily, so the gap is itself reportable: it means the public count series has not been refreshed since the 15 August report clock, and any apparent flattening in the trajectory over 16 to 19 August is an absence of reporting rather than an observed change in transmission.
- **Independent corroboration at the same cut.** The INRB/INSP/UMIE release of 19 August (`build-2026-08-19-13ed921`) reports DRC cumulative confirmed 4945 and confirmed deaths 2325 at data date 15 August, matching the DRC components of the country-scope endpoints above. It is archived as a source-review comparator: its per-health-zone table is not promoted into the published zone allocation, which continues to come from the reviewed INSP per-zone table.
