...
The current public artifact is a 13 September 2026 analytic snapshot. National headlines and the per-health-zone allocation come from visually reviewed INSP SitRep #122, whose report/data clock is 13 September 2026. **SitRep #122 reports 7258 confirmed cases and 3510 confirmed deaths in DRC.** The footprint remains **62 of 167 health zones** across seven provinces. Based on the public source package in this repo:

- **7278 confirmed cases** as the current country-scope endpoint in [`data/public_snapshot.json`](data/public_snapshot.json): **7258 DRC + 20 Uganda**. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **3512 confirmed deaths** as the country-scope endpoint: **3510 DRC + 2 Uganda**; country-scope recovered is **1737** (**1726 DRC + 11 Uganda**).
- **7278 confirmed cases country-scope, of which 7258 confirmed cases are officially zone-attributed across 62 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv), using the reviewed INSP per-zone table. The **20 confirmed cases** of source-attribution lag are exactly the separately sourced Uganda anchor. SitRep #122's named zone rows sum exactly to the DRC headline, while its printed 442-death unventilated residual is kept out of mapped health zones.
...
- **905 people in isolation/CTEs at 2026-09-13** on the current province/national response axis. Six province stocks close exactly to the headline; Sud-Ubangi publishes no care block. The national stock is direct, but INSP does not publish a complete national capacity denominator; it remains unset.
- **21935 of 27905 contacts seen (78.6%)** on the printed national line. Six reporting-province rows sum exactly to that value; Sud-Kivu is not published, not zero.
- **SitRep #122 splits same-day deaths across community and intra-CTE components.** It advances by 35 confirmed deaths (23 community + 12 intra-CTE) and 58 confirmations. The combined death value closes on cumulative movement; the community-only cover value is a subset.
