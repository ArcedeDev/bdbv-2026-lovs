...
The current public artifact is a 29 August 2026 analytic snapshot. National headlines and the per-health-zone allocation come from visually reviewed INSP SitRep #107, whose report/data clock is 29 August 2026 and whose official PDF was published on 31 August. **SitRep #107 reports 6041 confirmed cases and 2911 confirmed deaths in DRC.** The outbreak's official geography covers 60 of 151 health zones across six DRC provinces; SitRep #107 reports no further newly affected health zone. Based on the public source package in this repo:

- **6061 confirmed cases** as the current country-scope endpoint in [`data/public_snapshot.json`](data/public_snapshot.json): **6041 DRC + 20 Uganda**. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **2913 confirmed deaths** as the country-scope endpoint: **2911 DRC + 2 Uganda**; country-scope recovered is **1377** (**1366 DRC + 11 Uganda**).
- **6061 confirmed cases country-scope, of which 6041 confirmed cases are officially zone-attributed across 60 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv), using the reviewed INSP per-zone table. The **20 confirmed cases** of source-attribution lag are exactly the separately sourced Uganda anchor. SitRep #107's named zone rows sum exactly to the DRC headline, while its printed 314-death unventilated residual is kept out of mapped health zones.
...
- **SitRep #107 splits same-day deaths across community and intra-CTE components.** It advances by 49 confirmed deaths (41 community + 8 intra-CTE) and 96 confirmations. The combined death value closes on cumulative movement and is promoted; community-only headline snippets remain subsets, not the full daily death count.
