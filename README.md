...
The current public artifact is a 19 July 2026 analytic snapshot: headline counts are dated 19 July 2026 (reviewed INSP SitRep #066 promotion) and DRC INSP per-health-zone attribution is current to 19 July 2026 from the reviewed SitRep #066 Tableau 2 full cumulative zone table. Based on the public source package in this repo:

- **2443 confirmed cases** as the current country-scope confirmed endpoint in [`data/public_snapshot.json`](data/public_snapshot.json), composed of **2423 confirmed cases** across the five affected DRC provinces (Ituri, Nord-Kivu, Sud-Kivu, Haut-Uele, Tshopo) plus the 20-case Uganda anchor. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **969 confirmed deaths** as the cumulative confirmed-death endpoint.
- **2406 confirmed cases officially zone-attributed across 47 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv) are the primary descriptive spread signal. The 2443-case headline leaves **37 confirmed cases unallocated** to a public health-zone row: 17 DRC Ituri confirmed cases are explicitly unventilated (awaiting health-zone identification), and 20 Uganda confirmed cases have public country/district reporting but no promoted Uganda health-zone source-load. ONE new affected health zone was notified this cycle: Adja, in Aru Territory, northern Ituri, which entered the roster at 10 confirmed cases and 1 death in a single cycle rather than as a single index case. The roster moves to 47 health zones touched, 46 of them carrying active transmission because Miti-Murhesa (Sud-Kivu) remains in residual surveillance. Adja is rendered at its own GRID3 v8.0 polygon centroid and is not collapsed into a neighbouring zone.
- **439 suspected cases in isolation at 2026-07-19** in the separate `operational_status` block. SitRep #066 publishes a fresh Tableau 6 patient-movement table: 734 in isolation at 78.2% occupancy over 939 beds, whose confirmed/suspect split (295 / 439) closes exactly against the census and whose bed identity closes at every level (720 J-1 + 108 admissions - 94 exits = 734). The under-investigation stock and total active suspected queue are not republished, and those values are not fabricated. This is a point-in-time operational caseload, national-only, not a cumulative case count, and never added to confirmed.
...
For the current contract, the 37 confirmed cases not yet officially zone-attributed are treated as source-attribution lag, not as hidden model redistribution. Confirmed-death attribution retains the 1-3 week trailing-note disclosure; SitRep #066 prints a full cumulative per-zone table for the coherent 47-zone vector, with the 17 Ituri unventilated cases held in the explicit unallocated residual. The corridor surface is a descriptive 421-corridor watchlist over the reviewed source-load vector (up from 412 because the new Adja source zone adds nine candidate-target edges); its adjusted 50% interval ranges span 0.5-98.0% lower and 1.5-100.0% upper, and it is not deployment ranking or a forecast.
