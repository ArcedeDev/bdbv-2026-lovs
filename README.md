...
The current public artifact is a 9 September 2026 analytic snapshot. National headlines and the per-health-zone allocation come from visually reviewed INSP SitRep #118, whose report/data clock is 9 September 2026 and whose official PDF is dated 10 September. **SitRep #118 reports 6942 confirmed cases and 3349 confirmed deaths in DRC.** The outbreak's official geography covers 61 of 151 health zones across six DRC provinces, unchanged this cut. Based on the public source package in this repo:

- **6962 confirmed cases** as the current country-scope endpoint in [`data/public_snapshot.json`](data/public_snapshot.json): **6942 DRC + 20 Uganda**. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **3351 confirmed deaths** as the country-scope endpoint: **3349 DRC + 2 Uganda**; country-scope recovered is **1658** (**1647 DRC + 11 Uganda**).
- **6962 confirmed cases country-scope, of which 6942 confirmed cases are officially zone-attributed across 61 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv), using the reviewed INSP per-zone table. The **20 confirmed cases** of source-attribution lag are exactly the separately sourced Uganda anchor. SitRep #118's named zone rows sum exactly to the DRC headline, while its printed 415-death unventilated residual is kept out of mapped health zones.
...
- **833 people in isolation/CTEs at 2026-09-08**, carried on the current province/national response axis. The national stock is direct, but INSP does not publish a complete national capacity denominator; it remains unset. SitRep #117 publishes no national confirmed-versus-suspected split, so the status categories remain unclassified. The census is point-in-time operational state, not a cumulative case count, and is never added to confirmed.
- **21723 of 25108 contacts seen (86.5%)** on the five printed province rows. Sud-Kivu did not report; missing province values remain null rather than being converted to zero. The province rows reproduce the edition's own printed 86.5%. The changing denominator means the contact stock is not treated as a fixed cohort.
