...
The current public artifact is a 3 August 2026 analytic snapshot: headline counts and the DRC INSP per-health-zone attribution come from the visually reviewed SitRep #081 tables. **The 51 printed rows close to 3874 confirmed cases and 1749 confirmed deaths in DRC. The official Table 1 reports 1751 DRC deaths, but the North Kivu narrative and zone rows account for only 288 of its published 290 deaths, leaving two deaths unallocated rather than assigned without evidence. The footprint remains 51 zones, with a 24-hour movement of +72 confirmed cases and +44 confirmed deaths.** Based on the public source package in this repo:

- **3894 confirmed cases** as the current country-scope endpoint in [`data/public_snapshot.json`](data/public_snapshot.json): **3874 DRC + 20 Uganda**. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **1753 confirmed deaths** as the country-scope endpoint: **1751 DRC + 2 Uganda**.
- **3874 confirmed cases officially zone-attributed across 51 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv). The 20-case source-attribution lag against the country-scope headline is exactly the separately sourced Uganda country anchor; no DRC confirmed case is unallocated this cycle. Death attribution separately retains the two-death North Kivu source residual described above.
- **356 suspected cases in isolation at 2026-08-03** in the separate `operational_status` block. The source also classifies 330 isolated patients as confirmed but does not split Haut-Uele's remaining 31 patients by status, so the package preserves them as unclassified; the three components close to the national census of 717 without inference. North Kivu reports **196/141 beds (139.0%)** and Ituri **471/833 (56.5%)**. A national occupancy rate is not calculated because the source's capacity and status partitions are incomplete. This is a point-in-time operational caseload, not a cumulative case count, and is never added to confirmed.
