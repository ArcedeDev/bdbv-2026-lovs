...
The current public artifact is a 7 September 2026 analytic snapshot. National headlines and the per-health-zone allocation come from visually reviewed INSP SitRep #116, whose report/data clock is 7 September 2026 and whose official PDF reached insp.cd on 9 September. **SitRep #116 reports 6757 confirmed cases and 3267 confirmed deaths in DRC.** The outbreak's official geography covers 61 of 151 health zones across six DRC provinces, unchanged this cut. Based on the public source package in this repo:

- **6777 confirmed cases** as the current country-scope endpoint in [`data/public_snapshot.json`](data/public_snapshot.json): **6757 DRC + 20 Uganda**. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **3269 confirmed deaths** as the country-scope endpoint: **3267 DRC + 2 Uganda**; country-scope recovered is **1601** (**1590 DRC + 11 Uganda**).
- **6777 confirmed cases country-scope, of which 6757 confirmed cases are officially zone-attributed across 61 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv), using the reviewed INSP per-zone table. The **20 confirmed cases** of source-attribution lag are exactly the separately sourced Uganda anchor. SitRep #116's named zone rows sum exactly to the DRC headline, while its printed 387-death unventilated residual is kept out of mapped health zones.
- The descriptive **547-corridor watchlist** uses only those official source-zone rows; it carries a **0.5-100.0% lower** and **1.5-100.0% upper** adjusted-50 range as source-load context, not as an action-priority list or forecast.
- **813 people in isolation/CTEs at 2026-09-07**, carried on the current province/national response axis. The national stock is direct, but INSP does not publish a complete national capacity denominator; it remains unset. SitRep #116 publishes no national confirmed-versus-suspected split, so the status categories remain unclassified. The census is point-in-time operational state, not a cumulative case count, and is never added to confirmed.
- **21359 of 24179 contacts seen (88.3%)** on the five printed province rows. Sud-Kivu did not report; missing province values remain null rather than being converted to zero. The printed national denominator of 24719 is not used: the province rows sum to 24179, and only 24179 reproduces the edition's own printed 88.3%. The changing denominator means the contact stock is not treated as a fixed cohort.
- **SitRep #113 splits same-day deaths across community and intra-CTE components.** It advances by 39 confirmed deaths (30 community + 9 intra-CTE) and 86 confirmations. The combined death value closes on cumulative movement and is promoted; community-only headline snippets remain subsets, not the full daily death count.
