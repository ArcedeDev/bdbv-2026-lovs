...
The current public artifact is a 17 July 2026 analytic snapshot: headline counts are dated 17 July 2026 (reviewed INSP SitRep #064 promotion) and DRC INSP per-health-zone attribution is current to 17 July 2026, carried forward from the reviewed SitRep #063 zone table by the SitRep #064 per-zone day columns. Based on the public source package in this repo:

- **2287 confirmed cases** as the current country-scope confirmed endpoint in [`data/public_snapshot.json`](data/public_snapshot.json), composed of **2267 confirmed cases** across the five affected DRC provinces (Ituri, Nord-Kivu, Sud-Kivu, Haut-Uele, Tshopo) plus the 20-case Uganda anchor. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **895 confirmed deaths** as the cumulative confirmed-death endpoint.
- **2250 confirmed cases officially zone-attributed across 46 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv) are the primary descriptive spread signal. The 2287-case headline leaves **37 confirmed cases unallocated** to a public health-zone row: 17 DRC Ituri confirmed cases are explicitly unventilated (awaiting health-zone identification), and 20 Uganda confirmed cases have public country/district reporting but no promoted Uganda health-zone source-load. No new affected health zone was notified this cycle; the roster holds at 46 health zones touched, 45 of them carrying active transmission because Miti-Murhesa (Sud-Kivu) reached 52 days with no new confirmed case and remains in residual surveillance.
- **470 suspected cases in isolation at 2026-07-17** in the separate `operational_status` block. SitRep #064 publishes a fresh Tableau 6 patient-movement table: 722 in isolation at 85.9% occupancy over 841 beds, whose bed identity closes exactly (721 J-1 + 119 admissions - 118 exits = 722). Its printed confirmed/suspect split (252 / 460) sums to 712 against that same 722 census, so the corroborated census is promoted and the 10-case residual is carried on the suspect side (252 / 470), with the printed 460 preserved as a source conflict. The under-investigation stock and total active suspected queue are not republished, and those values are not fabricated. This is a point-in-time operational caseload, national-only, not a cumulative case count, and never added to confirmed.
...
For the current contract, the 37 confirmed cases not yet officially zone-attributed are treated as source-attribution lag, not as hidden model redistribution. Confirmed-death attribution retains the 1-3 week trailing-note disclosure; SitRep #064 carries per-zone confirmed deaths forward for the coherent 46-zone vector, with the 17 Ituri unventilated cases held in the explicit unallocated residual. The corridor surface is a descriptive 412-corridor watchlist over the reviewed source-load vector; its adjusted 50% interval ranges span 0.5-98.4% lower and 1.5-100.0% upper, and it is not deployment ranking or a forecast.
