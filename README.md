...
The current public artifact is a 4 September 2026 analytic snapshot. National headlines and the per-health-zone allocation come from visually reviewed INSP SitRep #113, whose report/data clock is 4 September 2026 and whose official PDF was posted on 5 September. **SitRep #113 reports 6522 confirmed cases and 3134 confirmed deaths in DRC.** The outbreak's official geography covers 61 of 151 health zones across six DRC provinces; Kayna is newly affected in Nord-Kivu. Based on the public source package in this repo:

- **6542 confirmed cases** as the current country-scope endpoint in [`data/public_snapshot.json`](data/public_snapshot.json): **6522 DRC + 20 Uganda**. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **3136 confirmed deaths** as the country-scope endpoint: **3134 DRC + 2 Uganda**; country-scope recovered is **1527** (**1516 DRC + 11 Uganda**).
- **6542 confirmed cases country-scope, of which 6522 confirmed cases are officially zone-attributed across 61 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv), using the reviewed INSP per-zone table. The **20 confirmed cases** of source-attribution lag are exactly the separately sourced Uganda anchor. SitRep #113's named zone rows sum exactly to the DRC headline, while its printed 361-death unventilated residual is kept out of mapped health zones.
...
- **817 people in isolation/CTEs at 2026-09-04**, carried on the current province/national response axis. The national stock is direct, but INSP does not publish a complete national capacity denominator; it remains unset. SitRep #113 publishes no national confirmed-versus-suspected split, so the status categories remain unclassified. The census is point-in-time operational state, not a cumulative case count, and is never added to confirmed.
- **21909 of 25500 contacts seen (86.0%)** on the printed national row. Sud-Kivu did not report; missing province values remain null rather than being converted to zero. The changing denominator means the contact stock is not treated as a fixed cohort.
