...
The current public artifact is a 21 July 2026 analytic snapshot: headline counts are dated 21 July 2026 (reviewed INSP SitRep #068 promotion) and DRC INSP per-health-zone attribution is current to 21 July 2026 from the reviewed SitRep #068 Tableau 2 full cumulative zone table. Based on the public source package in this repo:

- **2556 confirmed cases** as the current country-scope confirmed endpoint in [`data/public_snapshot.json`](data/public_snapshot.json), composed of **2536 confirmed cases** across the five affected DRC provinces (Ituri, Nord-Kivu, Sud-Kivu, Haut-Uele, Tshopo) plus the 20-case Uganda anchor. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **1035 confirmed deaths** as the cumulative confirmed-death endpoint.
- **2519 confirmed cases officially zone-attributed across 47 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv) are the primary descriptive spread signal. The 2556-case headline leaves **37 confirmed cases unallocated** to a public health-zone row: 17 DRC Ituri confirmed cases are explicitly unventilated (awaiting health-zone identification), and 20 Uganda confirmed cases have public country/district reporting but no promoted Uganda health-zone source-load. NO new affected health zone was notified this cycle: the roster holds at 47 health zones touched, 46 of them carrying active transmission because Miti-Murhesa (Sud-Kivu) remains in residual surveillance at 55 days without a confirmed case. Every cumulative zone delta against the previous cycle equals that zone's own printed day column, so no row was silently restated. Note that Table 1's printed provincial total (2531) is a source arithmetic slip against its own rows, which sum to 2536; the coherent 2536 is promoted. Each zone is rendered at its own GRID3 v8.0 polygon centroid and is never collapsed into a neighbouring zone.
- **464 suspected cases in isolation at 2026-07-21** in the separate `operational_status` block. SitRep #068 publishes a fresh Tableau 6 patient-movement table: 738 in isolation at 86.9% occupancy over 849 beds, whose confirmed/suspect split (274 / 464) closes exactly against the census and whose bed identity closes at every level (732 J-1 + 92 admissions - 86 exits = 738). Ituri bed capacity is flat this cycle: the printed series runs 693, 693, 700, 706, 798, 709, 708 across SitReps 62-68, so SitRep #068's 708 sits beside SitRep #067's 709 and is essentially the value SitRep #066's 798 spike should have printed, retrospectively confirming that 798 as a one-cycle data-entry outlier now bracketed by two on-trend cycles. Occupancy holds its ~78% plateau (78.4% to 78.5%) and national capacity is flat (850 to 849 beds); Nord-Kivu's 129.1% (182/141) remains the one genuine saturation. The under-investigation stock and total active suspected queue are not republished, and those values are not fabricated. This is a point-in-time operational caseload, national-only, not a cumulative case count, and never added to confirmed.
