...
The current public artifact is a 16 June 2026 analytic snapshot: headline counts are dated 16 June 2026 (reviewed INSP SitRep #033 promotion) and DRC INSP per-health-zone attribution is dated 16 June 2026 from the reviewed INSP SitRep section 3.2 zone list. Based on the public source package in this repo:

- **894 confirmed cases** as the current country-scope confirmed endpoint in [`data/public_snapshot.json`](data/public_snapshot.json). Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **204 confirmed deaths** as the cumulative confirmed-death endpoint.
- **781 confirmed cases officially zone-attributed across 33 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv) are the primary descriptive spread signal. The 894-case headline leaves **113 confirmed cases unallocated** to a public health-zone row: 94 DRC confirmed cases are explicitly unventilated by health zone in SitRep #033, and 19 Uganda confirmed cases have public country/district reporting but no promoted Uganda health-zone source-load.
- **227 suspected cases in isolation at 2026-06-16** in the separate `operational_status` block. SitRep #033 does not republish the under-investigation stock or total active suspected queue; those values are not fabricated. This is a point-in-time operational caseload, national-only, not a cumulative case count, and never added to confirmed.
...
For the current contract, the 113 confirmed cases not yet officially zone-attributed are treated as source-attribution lag, not as hidden model redistribution. Confirmed-death attribution has an explicit 1-3 week lag disclosure while clinical review catches up; SitRep #033 does not publish current per-zone confirmed deaths, so those are not inferred. The corridor surface is a descriptive 295-corridor watchlist over the reviewed source-load vector; its adjusted 50% interval ranges span 0.5-76.2% lower and 1.5-98.4% upper, and it is not deployment ranking or a forecast.
