...
The current public artifact is a 14 September 2026 analytic snapshot. National headlines and the per-health-zone allocation come from visually reviewed INSP SitRep #123, whose report/data clock is 14 September 2026. **SitRep #123 reports 7345 confirmed cases and 3545 confirmed deaths in DRC.** The footprint remains **62 of 167 health zones** across seven provinces. Based on the public source package in this repo:

- **7365 confirmed cases** as the current country-scope endpoint in [`data/public_snapshot.json`](data/public_snapshot.json): **7345 DRC + 20 Uganda**. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **3547 confirmed deaths** as the country-scope endpoint: **3545 DRC + 2 Uganda**; country-scope recovered is **1764** (**1753 DRC + 11 Uganda**).
- **7365 confirmed cases country-scope, of which 7345 confirmed cases are officially zone-attributed across 62 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv), using the reviewed INSP per-zone table. The **20 confirmed cases** of source-attribution lag are exactly the separately sourced Uganda anchor. SitRep #123's named zone rows sum exactly to the DRC headline, while its printed 446-death unventilated residual is kept out of mapped health zones.
...
- **938 people in isolation/CTEs at 2026-09-14** on the current province/national response axis. Six province stocks close exactly to the headline; Sud-Ubangi publishes no care block. The national stock is direct, but INSP does not publish a complete national capacity denominator; it remains unset.
...
- **SitRep #123 splits same-day deaths across community and intra-CTE components.** It advances by 35 confirmed deaths (26 community + 9 intra-CTE) and 87 confirmations. The combined death value closes on cumulative movement; the community-only cover value is a subset.
