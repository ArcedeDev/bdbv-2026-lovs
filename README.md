...
The current public artifact is a 20 August 2026 analytic snapshot. INSP released SitReps #097 and #098 after the prior package, and the snapshot's `as_of` remains the last day the data actually covers, not merely the day the documents appeared. National headlines and the per-health-zone allocation both come from SitRep #098, whose report/data clock is 20 August 2026. **SitRep #098 reports 5375 confirmed cases and 2557 confirmed deaths in DRC.** The outbreak's official geography holds at 56 of 151 health zones across six DRC provinces; no new affected health zone is reported in this edition. Based on the public source package in this repo:

- **5395 confirmed cases** as the current country-scope endpoint in [`data/public_snapshot.json`](data/public_snapshot.json): **5375 DRC + 20 Uganda**. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **2559 confirmed deaths** as the country-scope endpoint: **2557 DRC + 2 Uganda**; country-scope recovered is **1178** (**1167 DRC + 11 Uganda**).
- **5395 confirmed cases country-scope, of which 5375 confirmed cases are officially zone-attributed across 56 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv), using the reviewed INSP per-zone table. The **20 confirmed cases** of source-attribution lag are exactly the separately sourced Uganda anchor. SitRep #098's named zone rows sum exactly to the DRC headline, while its printed 206-death unventilated residual is kept out of mapped health zones.
- The descriptive **502-corridor watchlist** uses only those official source-zone rows; it carries a **0.5-100.0% lower** and **1.4-100.0% upper** adjusted-50 range as source-load context, not as an action-priority list or forecast.
- **737 people in isolation/CTEs at 2026-08-20**, carried on the current province/national response axis. SitRep #098 publishes no national confirmed-versus-suspected split, so all 737 remain unclassified by case status. The current printed province care blocks close when missing Haut-Uele care data is left null rather than set to zero: Ituri 536, Nord-Kivu 169, Tshopo 6, Bas-Uele 1 and Sud-Kivu 25 sum to 737. The census is point-in-time operational state, not a cumulative case count, and is never added to confirmed.
- **19674 of 23001 contacts seen (85.5%)** on the printed national row. The national figures exclude the separately printed Bas-Uele row: adding Bas-Uele would yield 19784 of 23234. Per province on the printed table: Ituri 89.3%, Tshopo 100.0%, Nord-Kivu 82.1%, Haut-Uele 61.6%, and Bas-Uele 59.5%.
- **SitReps #097 and #098 still split their same-day deaths across community and intra-CTE components.** SitRep #097 advances by 40 confirmed deaths (23 community + 17 intra-CTE), and SitRep #098 advances by 41 (18 community + 23 intra-CTE). The combined values close on cumulative death movement and are promoted; community-only headline snippets are preserved as subsets, not as the full daily death count.
