...
- **4747 confirmed cases country-scope, of which 4727 confirmed cases are officially zone-attributed across 55 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv), using the reviewed INSP per-zone table. The **20 confirmed cases** of source-attribution lag are exactly the separately sourced Uganda anchor. SitRep #091's zone rows, province rows and printed national total agree on every measure, so unlike SitRep #089 this edition contributes no arithmetic excess of its own.
