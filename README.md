...
The current public artifact is a 25 August 2026 analytic snapshot. National headlines and the per-health-zone allocation come from visually reviewed INSP SitRep #103, whose report/data clock is 25 August 2026 and whose official PDF was published on 26 August. **SitRep #103 reports 5713 confirmed cases and 2744 confirmed deaths in DRC.** The outbreak's official geography covers 58 of 151 health zones across six DRC provinces after SitRep #102 added Ganga in Bas-Uele; SitRep #103 reports no further newly affected health zone. Based on the public source package in this repo:

- **5733 confirmed cases** as the current country-scope endpoint in [`data/public_snapshot.json`](data/public_snapshot.json): **5713 DRC + 20 Uganda**. Laboratory-confirmed cases are the only cumulative case metric this snapshot publishes.
- **2746 confirmed deaths** as the country-scope endpoint: **2744 DRC + 2 Uganda**; country-scope recovered is **1280** (**1269 DRC + 11 Uganda**).
- **5733 confirmed cases country-scope, of which 5713 confirmed cases are officially zone-attributed across 58 official source zones** in [`data/public_zone_counts_2026-05-29.csv`](data/public_zone_counts_2026-05-29.csv), using the reviewed INSP per-zone table. The **20 confirmed cases** of source-attribution lag are exactly the separately sourced Uganda anchor. SitRep #103's named zone rows sum exactly to the DRC headline, while its printed 280-death unventilated residual is kept out of mapped health zones.
- The descriptive **520-corridor watchlist** uses only those official source-zone rows; it carries a **0.5-100.0% lower** and **1.5-100.0% upper** adjusted-50 range as source-load context, not as an action-priority list or forecast.
- **770 people in isolation/CTEs at 2026-08-25**, carried on the current province/national response axis. SitRep #103 publishes no national confirmed-versus-suspected split, so all 770 remain unclassified by case status. The printed province care blocks close at 770: Ituri 488, Nord-Kivu 194, Tshopo 8, Haut-Uele 79 and Bas-Uele 1; Sud-Kivu remains null. The census is point-in-time operational state, not a cumulative case count, and is never added to confirmed.
- **18574 of 21959 contacts seen (84.6%)** on the printed national row. Haut-Uele was not reported and Bas-Uele follow-up was 48.2%; missing province values remain null rather than being converted to zero. The changing denominator means the contact stock is not treated as a fixed cohort.
- **SitReps #101 through #103 split same-day deaths across community and intra-CTE components.** SitRep #103 advances by 29 confirmed deaths (11 community + 18 intra-CTE) and 57 confirmations. The combined death value closes on cumulative movement and is promoted; community-only headline snippets remain subsets, not the full daily death count. The stale 5656 total repeated in SitRep #103's page-nine key-message box is quarantined because its cover, province table, zone table and narrative close at 5713.
