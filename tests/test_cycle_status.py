...
class BuildTests(unittest.TestCase):
    ...
    def test_build_with_fixtures(self):
        ...
        # #129 reports on; the next reviewed source publication is already
        # present, so a later completed source publication is due.
        self.assertEqual(
            status["publication_route"]["basis"],
            "latest_completed_source_publication_date",
        )
        self.assertTrue(status["readiness"]["snapshot_due"])
        self.assertEqual(status["analytic_data_date"], "2026-09-20")
