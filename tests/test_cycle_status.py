...
class BuildTests(unittest.TestCase):
    ...
    def test_build_with_fixtures(self):
        ...
        # (read-only). The current snapshot's analytic as_of is 2026-07-03
        # after the reviewed SitRep #050 endpoint. SitRep #050 (data date
        # 2026-07-03) was published 2026-07-04 19:32 UTC; a day later the
        # 2026-07-03 data is a completed prior-day source publication, so the
        # route reports the latest-completed-source basis and a snapshot is due.
        self.assertEqual(
            status["publication_route"]["basis"],
            "latest_completed_source_publication_date",
        )
        self.assertTrue(status["readiness"]["snapshot_due"])
        self.assertEqual(status["analytic_data_date"], "2026-07-03")
