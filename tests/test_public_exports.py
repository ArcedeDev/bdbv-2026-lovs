...
class TestPublicExports(unittest.TestCase):
    ...
    def test_public_snapshot_contains_partner_relevant_fields(self):
        ...
        self.assertEqual("2026-06-19", snapshot["data_as_of"])
    ...
    def test_zone_counts_publish_source_attributed_health_zone_rows(self):
        ...
        self.assertEqual("259", by_zone["bunia"]["confirmed"])
        ...
        self.assertEqual("inrb-sitrep-036-2026-06-19", by_zone["bunia"]["source_id"])
    ...
    def test_local_aggregate_review_consumer_is_read_only_and_grounded(self):
        ...
        self.assertIn("source-attributed confirmed total: 939", result.stdout)
        self.assertIn("headline confirmed total: 975", result.stdout)
        self.assertIn("documented attribution gap: 36", result.stdout)
        self.assertIn("health-zone rows: 34", result.stdout)
        for term in ("risk_adj", "risk_raw", "feature_weights", "posterior_parameters"):
            self.assertNotIn(term, result.stdout)
