...
class TestPublicExports(unittest.TestCase):
    ...
    def test_public_snapshot_contains_partner_relevant_fields(self):
        ...
        self.assertEqual("2026-07-16", snapshot["data_as_of"])
    ...
    def test_zone_counts_publish_source_attributed_health_zone_rows(self):
        ...
        self.assertEqual("581", by_zone["bunia"]["confirmed"])
        # Mahagi is the 46th zone, first confirmed at SitRep62 on the Goli-Mahagi/Arua
        # cross-border axis; it holds at 1 confirmed this cycle and carries its own GRID3
        # v8.0 polygon, not collapsed into a neighbouring zone.
        ...
        self.assertEqual("193", by_zone["bunia"]["confirmed_deaths"])
        self.assertNotIn("suspected", by_zone["bunia"])
        self.assertEqual("present_with_data", by_zone["bunia"]["source_row_status"])
        self.assertEqual("inrb-sitrep-063-2026-07-16", by_zone["bunia"]["source_id"])
        self.assertEqual("5", by_zone["drodro"]["confirmed"])
        self.assertEqual("3", by_zone["drodro"]["confirmed_deaths"])
        self.assertEqual("6", by_zone["fataki"]["confirmed"])
        self.assertEqual("14", by_zone["musienene"]["confirmed"])
        self.assertEqual("4", by_zone["musienene"]["confirmed_deaths"])
        self.assertEqual("1", by_zone["mabalako"]["confirmed"])
        self.assertEqual("42", by_zone["nia-nia"]["confirmed"])
    ...
    def test_public_summary_consumer_is_read_only_and_grounded(self):
        ...
        self.assertIn("confirmed cases: 2201", result.stdout)
    ...
    def test_public_methodology_review_consumer_is_read_only_and_grounded(self):
        ...
        self.assertIn("confirmed primary: 2201", result.stdout)
    ...
    def test_local_aggregate_review_consumer_is_read_only_and_grounded(self):
        ...
        self.assertIn("source-attributed confirmed total: 2164", result.stdout)
        self.assertIn("headline confirmed total: 2201", result.stdout)
        self.assertIn("documented attribution gap: 37", result.stdout)
        self.assertIn("health-zone rows: 46", result.stdout)
        for term in ("risk_adj", "risk_raw", "feature_weights", "posterior_parameters"):
            self.assertNotIn(term, result.stdout)
    ...
    def test_public_adaptation_package_is_self_serve_and_safe(self):
        ...
        self.assertEqual(46, len(local_input["health_zone_counts"]))
