...
class TestPublicExports(unittest.TestCase):
    ...
    def test_public_snapshot_contains_partner_relevant_fields(self):
        ...
        self.assertEqual("2026-07-30", snapshot["data_as_of"])
    ...
    def test_zone_counts_publish_source_attributed_health_zone_rows(self):
        ...
        self.assertEqual("880", by_zone["bunia"]["confirmed"])
        ...
        self.assertEqual("257", by_zone["bunia"]["confirmed_deaths"])
        self.assertNotIn("suspected", by_zone["bunia"])
        self.assertEqual("present_with_data", by_zone["bunia"]["source_row_status"])
        self.assertEqual("inrb-sitrep-077-2026-07-30", by_zone["bunia"]["source_id"])
        self.assertEqual("10", by_zone["drodro"]["confirmed"])
        self.assertEqual("6", by_zone["drodro"]["confirmed_deaths"])
        self.assertEqual("27", by_zone["fataki"]["confirmed"])
        self.assertEqual("39", by_zone["musienene"]["confirmed"])
        self.assertEqual("22", by_zone["musienene"]["confirmed_deaths"])
        self.assertEqual("1", by_zone["mabalako"]["confirmed"])
        self.assertEqual("95", by_zone["nia-nia"]["confirmed"])
    ...
    def test_public_summary_consumer_is_read_only_and_grounded(self):
        ...
        self.assertIn("confirmed cases: 3625", result.stdout)
    ...
    def test_public_methodology_review_consumer_is_read_only_and_grounded(self):
        ...
        self.assertIn("confirmed primary: 3625", result.stdout)
    ...
    def test_local_aggregate_review_consumer_is_read_only_and_grounded(self):
        ...
        self.assertIn("source-attributed confirmed total: 3605", result.stdout)
        self.assertIn("headline confirmed total: 3625", result.stdout)
        self.assertIn("documented attribution gap: 20", result.stdout)
        self.assertIn("health-zone rows: 49", result.stdout)
        for term in ("risk_adj", "risk_raw", "feature_weights", "posterior_parameters"):
            self.assertNotIn(term, result.stdout)
