...
class TestPublicExports(unittest.TestCase):
    ...
    def test_public_snapshot_contains_partner_relevant_fields(self):
        ...
        self.assertEqual("2026-09-16", snapshot["data_as_of"])
    ...
    def test_zone_counts_publish_source_attributed_health_zone_rows(self):
        ...
        self.assertEqual("1620", by_zone["bunia"]["confirmed"])
        # Mahagi advances to 3 confirmed at SitRep72 (its second case, notified in the
        # day column) and carries its own GRID3 v8.0 polygon, not collapsed into a
        # neighbouring zone.
        self.assertEqual("4", by_zone["mahagi-cod"]["confirmed"])
        # Adja is the 47th zone, newly affected at SitRep66 in Aru Territory, northern
        # Ituri. It entered the roster at 10 confirmed / 1 death in a single cycle rather
        # than as one index case, and like Mahagi carries its OWN GRID3 v8.0 polygon
        # centroid rather than being collapsed into a neighbouring zone.
        self.assertEqual("11", by_zone["adja"]["confirmed"])
        self.assertEqual("1", by_zone["adja"]["confirmed_deaths"])
        ...
        self.assertEqual("511", by_zone["bunia"]["confirmed_deaths"])
        self.assertNotIn("suspected", by_zone["bunia"])
        self.assertEqual("present_with_data", by_zone["bunia"]["source_row_status"])
        self.assertEqual("inrb-sitrep-125-2026-09-16", by_zone["bunia"]["source_id"])
        self.assertEqual("15", by_zone["drodro"]["confirmed"])
        self.assertEqual("7", by_zone["drodro"]["confirmed_deaths"])
        self.assertEqual("80", by_zone["fataki"]["confirmed"])
        self.assertEqual("30", by_zone["fataki"]["confirmed_deaths"])
        self.assertEqual("118", by_zone["musienene"]["confirmed"])
        self.assertEqual("64", by_zone["musienene"]["confirmed_deaths"])
        self.assertEqual("22", by_zone["mabalako"]["confirmed"])
        self.assertEqual("253", by_zone["nia-nia"]["confirmed"])
        self.assertEqual("6", by_zone["mutwanga"]["confirmed"])
        self.assertEqual("18", by_zone["biena"]["confirmed"])
        self.assertEqual("10", by_zone["manguredjipa"]["confirmed"])
        self.assertEqual("3", by_zone["ganga"]["confirmed"])
        self.assertEqual("2", by_zone["mutwanga"]["confirmed_deaths"])
        self.assertEqual("1", by_zone["buta"]["confirmed"])
        self.assertEqual("1", by_zone["tshopo-cod"]["confirmed"])
    ...
    def test_public_calibration_status_summarizes_blocks(self):
        ...
        self.assertEqual(87, status["ledger_rows"])
        # The first four blocks have passed their resolution dates. Block 4
        ...
        # they are neither open nor scored. The 2026-09-01 group (Blocks 5, 6
        # and 7, 31 pins) is open until 2026-10-01, and states that it was
        # first published on 2026-09-17, after its pin date.
        ...
        self.assertEqual(5, len(status["blocks"]))
        ...
        self.assertTrue(all("first_published_at" not in block for block in status["blocks"][:-1]))
    ...
    def test_public_summary_consumer_is_read_only_and_grounded(self):
        ...
        self.assertIn("confirmed cases: 7495", result.stdout)
        self.assertIn("health-zone rows: 62", result.stdout)
    ...
    def test_public_methodology_review_consumer_is_read_only_and_grounded(self):
        ...
        self.assertIn("confirmed primary: 7495", result.stdout)
    ...
    def test_local_aggregate_review_consumer_is_read_only_and_grounded(self):
        ...
        self.assertIn("source-attributed confirmed total: 7475", result.stdout)
        self.assertIn("headline confirmed total: 7495", result.stdout)
        self.assertIn("documented attribution gap: 20", result.stdout)
        self.assertIn("health-zone rows: 62", result.stdout)
        for term in ("risk_adj", "risk_raw", "feature_weights", "posterior_parameters"):
            self.assertNotIn(term, result.stdout)
    ...
    def test_calibration_record_inspector_is_read_only_and_grounded(self):
        ...
        self.assertIn("commitments: 87", result.stdout)
        self.assertIn("verified pre-registration hash: 87/87", result.stdout)
    ...
    def test_public_adaptation_package_is_self_serve_and_safe(self):
        ...
        self.assertEqual(62, len(local_input["health_zone_counts"]))
        ...
        # The operational axis appears only when the edition publishes a NATIONAL
        # suspected subtotal. SitRep 92 restored one (461); SitRep 93 publishes the
        # suspected split per province only, and those sum to 466 against its own
        # printed national census of 730, so no national subtotal is derived and the
        # block is absent. Presence is edition-dependent; the invariants are not.
        # Whenever the block IS carried it must reach the adaptation example intact
        # and stay explicitly non-summable, because it is operational response status,
        # not a cumulative case class and not a C2 denominator.
        if "operational_status" in snapshot:
            self.assertIn("operational_status", local_input)
            self.assertFalse(local_input["operational_status"]["summable_into_confirmed"])
            in_isolation = snapshot["operational_status"].get("suspected_in_isolation")
            if in_isolation is not None:
                self.assertEqual(
                    in_isolation["value"],
                    local_input["operational_status"]["suspected_in_isolation"]["value"],
                )
        else:
            self.assertNotIn("operational_status", local_input)
        # SitRep 92 published a suspected split, carried as operational_status; SitRep 93
        # publishes only the isolation census, carried as care_census. Exactly one axis is
        # present, and whichever it is must be explicitly non-summable into confirmed.
        axes = [k for k in ("operational_status", "care_census") if k in local_input]
        self.assertEqual(1, len(axes), f"expected exactly one operational axis, got {axes}")
        self.assertFalse(local_input[axes[0]]["summable_into_confirmed"])
