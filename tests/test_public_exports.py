...
class TestPublicExports(unittest.TestCase):
    ...
    def test_public_snapshot_contains_partner_relevant_fields(self):
        ...
        self.assertEqual("2026-08-07", snapshot["data_as_of"])
    ...
    def test_zone_counts_publish_source_attributed_health_zone_rows(self):
        ...
        self.assertEqual("967", by_zone["bunia"]["confirmed"])
        ...
        self.assertEqual("297", by_zone["bunia"]["confirmed_deaths"])
        self.assertNotIn("suspected", by_zone["bunia"])
        self.assertEqual("present_with_data", by_zone["bunia"]["source_row_status"])
        self.assertEqual("inrb-sitrep-083-2026-08-05", by_zone["bunia"]["source_id"])
        self.assertEqual("11", by_zone["drodro"]["confirmed"])
        self.assertEqual("6", by_zone["drodro"]["confirmed_deaths"])
        self.assertEqual("45", by_zone["fataki"]["confirmed"])
        self.assertEqual("24", by_zone["fataki"]["confirmed_deaths"])
        self.assertEqual("47", by_zone["musienene"]["confirmed"])
        self.assertEqual("26", by_zone["musienene"]["confirmed_deaths"])
        self.assertEqual("1", by_zone["mabalako"]["confirmed"])
        self.assertEqual("114", by_zone["nia-nia"]["confirmed"])
    ...
    def test_public_summary_consumer_is_read_only_and_grounded(self):
        ...
        self.assertIn("confirmed cases: 4229", result.stdout)
    ...
    def test_public_methodology_review_consumer_is_read_only_and_grounded(self):
        ...
        self.assertIn("confirmed primary: 4229", result.stdout)
        self.assertIn("documented attribution gap: 176", result.stdout)
    ...
    def test_local_aggregate_review_consumer_is_read_only_and_grounded(self):
        ...
        self.assertIn("headline confirmed total: 4229", result.stdout)
        self.assertIn("documented attribution gap: 176", result.stdout)
        self.assertIn("health-zone rows: 53", result.stdout)
        for term in ("risk_adj", "risk_raw", "feature_weights", "posterior_parameters"):
            self.assertNotIn(term, result.stdout)
    ...
    def test_public_adaptation_package_is_self_serve_and_safe(self):
        ...
        # example carries the 595 people as wholly unclassified and not summable.
        self.assertNotIn("operational_status", snapshot)
        self.assertNotIn("operational_status", local_input)
        self.assertEqual(595, local_input["care_census"]["patients_in_isolation_or_cte"])
        self.assertEqual(595, local_input["care_census"]["unclassified_by_case_status"])
