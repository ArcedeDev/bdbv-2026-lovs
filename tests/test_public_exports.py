...
class TestPublicExports(unittest.TestCase):
    ...
    def test_public_snapshot_contains_partner_relevant_fields(self):
        ...
        self.assertEqual("2026-09-01", snapshot["data_as_of"])
    ...
    def test_zone_counts_publish_source_attributed_health_zone_rows(self):
        ...
        self.assertEqual("1413", by_zone["bunia"]["confirmed"])
        ...
        self.assertEqual("442", by_zone["bunia"]["confirmed_deaths"])
        self.assertNotIn("suspected", by_zone["bunia"])
        self.assertEqual("present_with_data", by_zone["bunia"]["source_row_status"])
        self.assertEqual("inrb-sitrep-110-2026-09-01", by_zone["bunia"]["source_id"])
        self.assertEqual("14", by_zone["drodro"]["confirmed"])
        self.assertEqual("7", by_zone["drodro"]["confirmed_deaths"])
        self.assertEqual("77", by_zone["fataki"]["confirmed"])
        self.assertEqual("30", by_zone["fataki"]["confirmed_deaths"])
        self.assertEqual("83", by_zone["musienene"]["confirmed"])
        self.assertEqual("47", by_zone["musienene"]["confirmed_deaths"])
        self.assertEqual("6", by_zone["mabalako"]["confirmed"])
        self.assertEqual("216", by_zone["nia-nia"]["confirmed"])
        self.assertEqual("2", by_zone["mutwanga"]["confirmed"])
        self.assertEqual("1", by_zone["biena"]["confirmed"])
        self.assertEqual("2", by_zone["manguredjipa"]["confirmed"])
    ...
    def test_public_summary_consumer_is_read_only_and_grounded(self):
        ...
        self.assertIn("confirmed cases: 6270", result.stdout)
    ...
    def test_public_methodology_review_consumer_is_read_only_and_grounded(self):
        ...
        self.assertIn("confirmed primary: 6270", result.stdout)
    ...
    def test_local_aggregate_review_consumer_is_read_only_and_grounded(self):
        ...
        self.assertIn("source-attributed confirmed total: 6250", result.stdout)
        self.assertIn("headline confirmed total: 6270", result.stdout)
        self.assertIn("documented attribution gap: 20", result.stdout)
        self.assertIn("health-zone rows: 60", result.stdout)
        for term in ("risk_adj", "risk_raw", "feature_weights", "posterior_parameters"):
            self.assertNotIn(term, result.stdout)
