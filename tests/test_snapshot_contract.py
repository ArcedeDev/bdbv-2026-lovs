...
class TestSnapshotContract(unittest.TestCase):
    ...
    def test_contract_captures_current_june4_partition(self):
        contract = snapshot_contract.build_contract(self._snapshot())

        self.assertEqual(467, contract["confirmed_case_partition"]["headline_confirmed_total"])
        ...
        # The SitRep21 headline is fresher than the per-zone table, so
        # unallocated = 467 headline - 243 zone-attributed = 224.
        self.assertEqual(243, contract["confirmed_case_partition"]["zone_attributed_confirmed_total"])
        self.assertEqual(224, contract["confirmed_case_partition"]["unallocated_confirmed_total"])
