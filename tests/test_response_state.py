...
class TestFrozenInvariants(unittest.TestCase):
    def test_headline_1812_627_current(self) -> None:
        ...
        self.assertEqual(live["reported_counts"]["confirmed"]["primary"], 2031)
        self.assertEqual(live["reported_deaths"]["confirmed"]["primary"], 756)
...
class TestGeneratedPublicSnapshotResponseState(unittest.TestCase):
    ...
    def test_generated_snapshot_clock_distinct_from_headline(self) -> None:
        ...
        self.assertEqual(self.response["data_as_of"], "2026-07-13")
        self.assertEqual(self.response["per_zone_data_as_of"], "2026-05-30")
        self.assertTrue(self.snapshot["as_of"].startswith("2026-07-13"))
